...
import { applyEtfProxyStatsToStocks, resolveEtfProxyStocks } from "./portfolio/etf-proxy.js";
...
async function enrichTickerEtfEntry(
  ...
  if (!hasEtfSnapshotSignal(stockEntry.indicators) && !(await isEtfTicker(ticker, stockEntry))) {
  ...
  const snapshotHasData =
    snapshotCache.snapshot.holdings.length > 0 || snapshotCache.snapshot.sectors.length > 0;
  const indicators =
...
async function buildStandaloneTickerRow({
  ...
  const resolved = await resolveTickerStats(store, tickerSymbol, source, stockEntry);
...
async function loadTickerContext(
  ...
  return {
    ticker: tickerSymbol,
    position: positions.find((row) => normalizeTicker(row.ticker) === tickerSymbol) ?? {
      ticker: tickerSymbol,
      quantity: 0,
      strategy: null,
    },
