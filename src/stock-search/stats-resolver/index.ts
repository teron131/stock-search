...
import type { FamilyResolution, StatsResolutionMode, StatsResolutionResult } from "./types.js";

export type { StatsResolutionMode, StatsResolutionResult } from "./types.js";
export { isLiveStatsUnavailableError, LiveStatsUnavailableError } from "./types.js";
...
export async function resolveTickerStatsMap(
  ...
  const prefetchedStocks = stockEntries ?? (await store.loadStocksByTickers(unique));
  const results = await mapWithConcurrency(
    ...
    async (ticker) =>
      [
        ticker,
        await resolveTickerStats(store, ticker, mode, prefetchedStocks[ticker] ?? null, options),
      ] as const,
  );
