...
export function sourceFieldPolicies(
	...groups: SourceFieldPolicyGroup[]
): SourceFieldPolicy[] {
...
export function mergeSourceFields(
	input: SourceMergeInput,
): Record<string, unknown> {
	const fieldsBySource = new Map(
		input.sources.map((snapshot) => [snapshot.source, snapshot.fields]),
	);
	const defaultSources =
		input.defaultSources ?? input.sources.map((snapshot) => snapshot.source);
	...
	for (const field of input.fields) {
		...
		output[field] = firstValue(
			field,
			policy?.sources ?? defaultSources,
			fieldsBySource,
		);
	}
