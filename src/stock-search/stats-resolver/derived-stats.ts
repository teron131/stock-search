...
function assignDerivedPeg(indicators: Record<string, unknown>): void {
  ...
  indicators.peg_source = derivedPeg == null ? null : PEG_SOURCE_DERIVED_NTM_FORWARD_PE;
}

function isDerivedPegValue(indicators: Record<string, unknown>, peg: number): boolean {
  const derivedPeg = derivePegFromNtmForwardPe(indicators);
  return derivedPeg != null && Math.abs(derivedPeg - peg) <= PEG_SOURCE_MATCH_TOLERANCE;
}

export function derivePegFromNtmForwardPe(indicators: Record<string, unknown>): number | null {
 ...
 */
export function blendImpliedGrowthPeg(inputs: PegBlendSourceInput[]): PegBlendResult {
  ...
  const impliedGrowth = mean(sources.map((source) => source.peg_implied_growth_percent));
  const minGrowth = Math.min(...sources.map((source) => source.peg_implied_growth_percent));
  const maxGrowth = Math.max(...sources.map((source) => source.peg_implied_growth_percent));
  ...
  return {
    ...
    confidence: growthSpread < 0.1 ? "high" : growthSpread < 0.2 ? "medium" : "low",
    sources,
  };
}

export function applyDerivedPegFallback(indicators: Record<string, unknown>): void {
...
export function applySourcePegFallback(
  ...
  if (result.peg != null && result.source != null) {
    ...
    indicators.peg_source = result.mode === "blend" ? PEG_SOURCE_BLEND : result.source;
    return;
  }
...
export function applyCachedPegFallback(indicators: Record<string, unknown>): void {
