...
import type { FamilyCacheEntry, FamilyResolution, StatsResolutionMode } from "./types.js";
...
async function refreshFamilyRow(
  ...
  if (family === "statistics") {
    const [yahoo, stockAnalysisStatistics, finvizStatistics] = await Promise.all([
      bundle.getYahooIndicators(),
  ...
  if (family === "financials") {
    const [yahoo, stockAnalysisStatistics, stockAnalysisFinancialsSnapshot, finvizStatistics] =
      await Promise.all([
        bundle.getYahooIndicators(),
        bundle.getStockAnalysisStatistics(),
    ...
    const stockAnalysisFinancials = {
      ...mergeStockAnalysisSnapshots(stockAnalysisStatistics, stockAnalysisFinancialsSnapshot),
...
async function refreshAndPersistFamily({
  ...
  const mergedIndicators = mergeFamilyRow(persistedRow, family, refreshedRow, refreshedAt);
...
async function queueRefresh(
  ...
  if (entry?.lastFailureAt != null && entry.lastFailureAt > Date.now() - cooldownMs) {
    return false;
  }
...
export async function resolveFamily({
  ...
  if (refreshDecision.action === "serve_stale_and_queue") {
    return {
      ...
      sourceTier: refreshDecision.sourceTier,
      timestamp: cached.timestamp,
      queuedRefresh: await queueRefresh(store, ticker, family, persistedRow, stockEntry),
    };
  }
