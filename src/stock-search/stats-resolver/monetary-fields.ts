...
const STOCKANALYSIS_STATEMENT_FALLBACK_FIELDS = ["revenue", "free_cash_flow"] as const;
  ...
  for (const field of FX_MONETARY_FIELDS) {
    if (fallbackFx != null || asNumber(output[field]) == null) {
      ...
      if (fallbackValue != null) {
        output[field] = fallbackFx != null ? Math.round(fallbackValue * fallbackFx) : fallbackValue;
