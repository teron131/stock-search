...
import { resolveTickerStatsMap, type StatsResolutionResult } from "../stats-resolver/index.js";
...
const ETF_PROXY_TIMESTAMP_FIELDS = ["statistics_fetched_at", "financials_fetched_at"] as const;
...
const ETF_PROXY_HARMONIC_MEAN_FIELDS = new Set<string>(["pe", "pe_forward", "ps", "ps_forward"]);
const ETF_PROXY_POSITIVE_RATIO_FIELDS = new Set<string>([...ETF_PROXY_HARMONIC_MEAN_FIELDS, "peg"]);
...
export function etfProxyResolutionForRows(
  ...
  const knownEtfs = new Set(etfPositions.map((position) => normalizeTicker(position.ticker)));
...
export function isStockLikeEtfRepresentativeTicker(ticker: string): boolean {
  ...
  const [exchangePrefix, tickerBody] = ticker.includes(":") ? ticker.split(":", 2) : ["", ticker];
...
export function buildEtfRepresentativePositions(
  ...
  for (const etfPosition of resolution.etfPositions) {
    ...
    for (const holding of snapshot?.holdings ?? []) {
      ...
      if (existing) {
        ...
        existing.etf_holding_weight = Math.max(existing.etf_holding_weight, holdingWeight);
  ...
  return [...representativesByTicker.values()].filter(
    (position) => position.etf_holding_notional_weight > ETF_LOOKTHROUGH_ROW_MIN_NOTIONAL_WEIGHT,
  );
}
...
function hasFreshEtfProxyStats(stockEntry: StockEntry | undefined, now: number): boolean {
...
export async function resolveEtfProxyStocks({
  ...
  const cachedStocks =
    missingTickers.length > 0 ? await store.loadStocksByTickers(missingTickers) : {};
  const stockEntries = {
    ...cachedStocks,
    ...knownStocks,
  };
  ...
  const now = Date.now();
  const normalRefresh = uniqueTickers(tickers.filter((ticker) => normalRefreshTickers.has(ticker)));
  const proxyRefresh = uniqueTickers(
    tickers.filter(
      (ticker) =>
        !normalRefreshTickers.has(ticker) && !hasFreshEtfProxyStats(stockEntries[ticker], now),
    ),
  );
