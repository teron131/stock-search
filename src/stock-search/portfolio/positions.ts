...
async function ensureValidNewTicker(ticker: string): Promise<void> {
  const indicators = await new YahooFinanceSource(ticker).getIndicatorsSnapshot();
...
export async function patchPortfolioPosition(
  ...
  const current = index >= 0 ? { ...positions[index] } : ({ ticker: tickerSymbol } as PositionRow);
  if (index < 0) {
  ...
  if (patch.quantity !== undefined) {
    ...
    current[POSITION_SOURCE_FIELD] =
      current.quantity > 0 ? POSITION_SOURCE_DASHBOARD_MANUAL : POSITION_SOURCE_DASHBOARD_WATCHLIST;
  }
