...
function scalarForColumn(value: Record<string, unknown>, column: StockScalarColumn): StorageValue {
...
function serializedIndicatorValue(indicators: Record<string, unknown>, columnName: string): string {
...
function stockFieldValues(
  ...
  return [
    ...
    ...STOCK_SCALAR_COLUMNS.map((column) =>
      scalarForColumn(column.group === "indicator" ? indicators : evaluation, column),
    ),
...
export function stockValues({
  ...
  const values = [ticker, ...stockFieldValues(normalizedIndicators, evaluation, labels)];
  if (updatedAt != null) {
...
export function stockFromRow(row: StockRow): StockEntry {
  const indicators: Record<string, unknown> = jsonParse<Record<string, unknown>>(
    row.indicator_extra,
    {},
  );
  const evaluation: Record<string, unknown> = jsonParse<Record<string, unknown>>(
...
export function positionFromRow(row: Record<string, unknown>): PositionRow | null {
  const ticker = normalizeTicker(row.ticker);
  if (!ticker) {
    return null;
  }
  ...
  const industryLabels = normalizeStringArray(jsonParse<unknown>(row.industry_labels, []));
...
export function portfolioStatsValues({
  ...
  const values: StorageValue[] = [
    key,
    ...PORTFOLIO_STAT_COLUMNS.map((column) => finiteNumberOrNull(portfolioStats[column])),
...
export function sectorSnapshotHeaderValues({
  ...
  const meta: Record<string, unknown> = isRecord(snapshot.meta) ? snapshot.meta : {};
  return [
    key,
...
function applyEvaluationReasons(row: StockRow, evaluation: Record<string, unknown>): void {
  for (const [scoreField, reasonsField] of STOCK_EVALUATION_REASON_COLUMNS) {
    const reasons = normalizeStringArray(jsonParse<unknown>(row[reasonsField], []));
    if (reasons.length > 0) {
...
function applyFutureOutlook(row: StockRow, evaluation: Record<string, unknown>): void {
...
function applySerializedIndicators(row: StockRow, indicators: Record<string, unknown>): void {
