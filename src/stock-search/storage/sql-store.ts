...
export abstract class SqlStore implements BackendStore {
  ...
  async savePortfolio({
    ...
    await this.batch([
      ...
      ...positions.flatMap((position, index) => this.positionStatement(key, position, index)),
    ]);
  }
  ...
  async savePositions(positions: PositionRow[], key = DEFAULT_STORAGE_KEY): Promise<void> {
    await this.ready();
    await this.batch([
      { sql: "DELETE FROM positions WHERE key = ?", params: [key] },
      ...positions.flatMap((position, index) => this.positionStatement(key, position, index)),
  ...
  async upsertStocks(
    ...
    await this.ready();
    const existingStocks = await this.loadStocksByTickers(rows.map((row) => row.ticker));
  ...
  async saveNews(rows: CachedNewsRow[], key = DEFAULT_STORAGE_KEY): Promise<void> {
    await this.ready();
    await this.batch([
  ...
  async deleteNewsByTickers(tickers: string[], key = DEFAULT_STORAGE_KEY): Promise<void> {
    await this.ready();
    const normalizedTickers = normalizeTickers(tickers);
    if (normalizedTickers.length === 0) {
      return;
  ...
  async loadSectorSnapshot(key = DEFAULT_STORAGE_KEY): Promise<StockAnalysisSectorSnapshot | null> {
    await this.ready();
  ...
  private positionStatement(key: string, position: PositionRow, sortIndex: number): SqlStatement[] {
