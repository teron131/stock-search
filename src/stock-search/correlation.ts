...
import { DEFAULT_EFFECTIVE_SLEEVE_CAP, MARKET_PROXY_TICKER } from "./correlation/constants.js";

export { DEFAULT_CORRELATION_TICKERS, HORIZONS, LOOKBACKS } from "./correlation/constants.js";
...
export { loadCorrelationPositions, resolveCorrelationTickers } from "./correlation/history.js";
...
import type { CorrelationReport, CorrelationReportOptions } from "./correlation/types.js";
...
export async function runCorrelationReport(
  ...
  const marketTicker = normalizeTicker(options.marketProxyTicker ?? MARKET_PROXY_TICKER);
  ...
  if (frame.height === 0) {
    throw new Error("No valid close price history available for requested tickers.");
  }
  ...
  const hasMarketHistory = frameRows.some((row) => isFiniteNumber(row.values.get(marketTicker)));
  const correlationTickers =
    marketTicker && hasMarketHistory ? [...activeTickers, marketTicker] : activeTickers;
  ...
  const normalResult = buildBlendedMatrix(correlationFrame, activeTickers, {
    blendWeightMode,
    correlationMode,
    marketProxyTicker: correlationMode === "market_neutral" ? marketTicker : null,
  });
  ...
  const stats = buildTickerStats(selectTickerRows(frame, activeTickers), activeTickers, names);
  return {
    ...
    sleeveWeightRecommendations: buildSleeveWeightRecommendations({
      ...
      tailRawCorrelationMatrix: tailRawMatrix.tickers.length > 0 ? tailRawMatrix : undefined,
      effectiveSleeveCap: options.effectiveSleeveCap ?? DEFAULT_EFFECTIVE_SLEEVE_CAP,
