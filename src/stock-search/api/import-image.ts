...
const PortfolioImageExtractionSchema = z
  .object({
    holdings: z.array(
      z.object({
        ticker: z.string().describe("Uppercase stock ticker symbol read from the image."),
        quantity: z.number().describe("Numeric share quantity read from the same holding row."),
      }),
    ),
  })
  .describe("[STRUCTURED OUTPUTS] Portfolio holdings extracted from an uploaded image.");
...
export async function importPortfolioImage(
  ...
  const merged = reconcileImportedHoldings(positions, extraction.holdings, strategy);
