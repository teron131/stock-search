...
function authErrorResponse(c: Context, message: string, status: 400 | 403 | 502): Response {
  clearAuthCookies(c);
...
function setSessionCookie(c: Context, user: Omit<SessionUser, "iat" | "exp">): void {
...
export async function authGuard(c: Context, next: Next) {
  ...
  if (isAppPagePath(c.req.path) && isPageNavigationRequest(c)) {
    ...
    return c.redirect(`${AUTH_LOGIN}?next=${encodeURIComponent(nextPath)}`, 307);
  }
...
function normalizeAllowedGoogleUser(
  ...
  if (!email || !subject || !userInfo.email_verified || email !== appConfig.allowedEmail) {
    return null;
  }
  return {
    email,
...
export async function handleCallback(c: Context): Promise<Response> {
  ...
  try {
    ...
    if (!sessionUser) {
      ...
      if (!userInfo.sub) {
        return authErrorResponse(c, "Google account is missing a subject.", 403);
      }
