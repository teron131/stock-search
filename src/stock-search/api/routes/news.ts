...
import { PortfolioNewsSummaryWriteSchema, PortfolioNewsWriteSchema } from "../../models/schemas.js";
...
function parseNewsMode(rawValue: string | undefined): newsPipeline.NewsFetchMode {
  const mode = String(rawValue || "raw-fast").trim();
  return NEWS_MODES.has(mode) ? (mode as newsPipeline.NewsFetchMode) : "raw-fast";
}
...
export function createNewsRouter(store: BackendStore): Hono {
  ...
  router.get(PORTFOLIO_NEWS, async (c) => {
    ...
    const portfolioNews = await loadPortfolioNews(store, parsePortfolioNewsKey(c.req.query("key")));
    return c.json(portfolioNews ?? null);
  });

  router.post(PORTFOLIO_NEWS, async (c) => {
    c.header("Cache-Control", "no-store");
    const input = PortfolioNewsWriteSchema.parse(await c.req.json().catch(() => null));
    return c.json(await savePortfolioNews(store, input));
  });

  router.get(PORTFOLIO_NEWS_SUMMARY, async (c) => {
    c.header("Cache-Control", "no-store");
    return c.json(await loadPortfolioNewsSummary(store, parsePortfolioNewsKey(c.req.query("key"))));
  });

  router.post(PORTFOLIO_NEWS_SUMMARY, async (c) => {
    c.header("Cache-Control", "no-store");
    const input = PortfolioNewsSummaryWriteSchema.parse(await c.req.json().catch(() => null));
    return c.json(await savePortfolioNewsSummary(store, input));
  });

  router.post(PORTFOLIO_NEWS_SUMMARIZE, async (c) => {
    c.header("Cache-Control", "no-store");
    const { rows, items } = parsePortfolioNewsSummaryPayload(await c.req.json().catch(() => null));
