...
export const PORTFOLIO_SCOPE_POLICIES: Record<PortfolioScope, PortfolioScopePolicy> = {
  portfolio_cached: {
    universe: "portfolio",
    statsMode: "cache",
    refreshIntent: "none",
...
export class RequestPolicy {
  ...
  isTickerSource(value: unknown): value is TickerSource {
    return (
      typeof value === "string" && (this.tickerSourceValues as readonly string[]).includes(value)
    );
  }

  isPortfolioScope(value: unknown): value is PortfolioScope {
    return (
      typeof value === "string" && (this.portfolioScopeValues as readonly string[]).includes(value)
    );
  }
