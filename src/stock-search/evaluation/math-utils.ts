...
function approximateErrorFunction(value: number): number {
  ...
  const y = 1 - ((((a5 * t + a4) * t + a3) * t + a2) * t + a1) * t * Math.exp(-absValue * absValue);
  return sign * y;
}
...
export function mapToCurveScore(
  ...
  const curveScale = (value <= inMedian ? inMedian - inMin : inMax - inMedian) / 3;
  const curvePosition = curveScale > 0 ? (value - inMedian) / curveScale : 0;
  const curvePercentile = 0.5 * (1 + approximateErrorFunction(curvePosition / Math.sqrt(2)));
  return clampScore(outMin + (outMax - outMin) * curvePercentile, clampMin, clampMax);
}
