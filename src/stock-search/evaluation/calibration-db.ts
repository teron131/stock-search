...
import { calibrationDbPath, resetScoreAnchorsCache, setCalibrationStatsRows } from "./anchors.js";
...
type SyncEvaluationCalibrationOptions = {
  ...
  logWarnings?: boolean;
  store?: Pick<BackendStore, "loadCalibrationStats" | "upsertCalibrationStatsRows">;
};
...
export function missingCalibrationScoreFields(indicators: Record<string, unknown>): string[] {
  return CALIBRATION_SCORE_FIELD_NAMES.filter((fieldName) => indicators[fieldName] == null);
}

export function isCompleteCalibrationScoreRow(indicators: Record<string, unknown>): boolean {
  return missingCalibrationScoreFields(indicators).length === 0;
}

export function latestCalibrationFetchedAt(indicators: Record<string, unknown>): string | null {
...
export function indicatorsAreNewer(
  ...
  const incomingLatest = parseTimestamp(latestCalibrationFetchedAt(incomingIndicators));
  const existingLatest = parseTimestamp(latestCalibrationFetchedAt(existingIndicators));
  return incomingLatest != null && (existingLatest == null || incomingLatest > existingLatest);
}
...
export function ensureCalibrationStatsTable(database: DatabaseSync): void {
  ...
  for (const definition of CALIBRATION_STATS_COLUMN_DEFINITIONS) {
    ...
    database.exec(`ALTER TABLE calibration_stats ADD COLUMN ${columnName} ${columnType.join(" ")}`);
  }
  database.exec(`
...
function runCalibrationStatsRowUpsert(
  ...
  const updates = CALIBRATION_STATS_COLUMN_NAMES.filter((columnName) => columnName !== "ticker")
...
async function syncEvaluationCalibrationRow(
  ...
  const indicators = mergeNonNullFields(existing?.indicators ?? {}, row.indicators);
