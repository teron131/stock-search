...
function roundScore(value: number | null): number | null {
  return value == null || !Number.isFinite(value) ? null : Number(value.toFixed(SCORE_DIGITS));
}
...
function setScoreIfPresent(target: Record<string, number>, key: string, value: unknown): void {
...
function setDerivedScore(target: Record<string, number>, key: string, value: number | null): void {
...
function setOptionalStoredScore(target: Record<string, number>, key: string, value: unknown): void {
  const score = optionalScore(value);
  if (score != null) {
    target[key] = score;
    return;
...
function analystTargetGap(indicatorRow: Record<string, unknown>): number | null {
...
function currentEvaluationScores(indicatorRow: Record<string, unknown>): Record<string, number> {
  const normalized: Record<string, number> = {};
  const quoteType = String(indicatorRow.quote_type ?? indicatorRow.equity_type ?? "")
    .trim()
    .toUpperCase();
  const hasProxyStats =
    Array.isArray(indicatorRow.proxied_stat_fields) && indicatorRow.proxied_stat_fields.length > 0;
  if ((quoteType === "ETF" || quoteType === "MUTUALFUND") && !hasProxyStats) {
    return normalized;
  }

  setDerivedScore(normalized, "quality_score", calculateQualitySignalScore(indicatorRow));
  setDerivedScore(normalized, "moat_score", calculateMoatSignalScore(indicatorRow));
  setDerivedScore(normalized, "valuation_score", calculateValuationScore(indicatorRow));
  setDerivedScore(
    normalized,
    "upside_score",
    calculateCombinedUpsideScore(
...
export function deriveEvaluationScores(
  ...
  setOptionalStoredScore(normalized, "llm_quality_score", data?.llm_quality_score);
...
function scoreAtLeast(value: number | null | undefined, minimum: number): boolean {
  return value != null && Number.isFinite(value) && value >= minimum;
}

function scoreAtMost(value: number | null | undefined, maximum: number): boolean {
...
function passesSatelliteGate(scores: Record<string, number | undefined>): boolean {
  return (
    scoreAtLeast(scores.overall_score, StrategyGateConfig.SATELLITE_OVERALL_MIN) &&
    scoreAtLeast(scores.valuation_score, StrategyGateConfig.SATELLITE_VALUATION_MIN) &&
    scoreAtLeast(scores.moat_score, StrategyGateConfig.SATELLITE_MOAT_MIN) &&
    scoreAtLeast(scores.quality_score, StrategyGateConfig.SATELLITE_QUALITY_MIN) &&
    (scoreAtLeast(scores.upside_score, StrategyGateConfig.SATELLITE_UPSIDE_MIN) ||
      scoreAtLeast(scores.tactical_score, StrategyGateConfig.SATELLITE_TACTICAL_MIN))
  );
}

function passesDefenseGate(scores: Record<string, number | undefined>): boolean {
  return (
    scoreAtLeast(scores.overall_score, StrategyGateConfig.DEFENSE_OVERALL_MIN) &&
    scoreAtLeast(scores.quality_score, StrategyGateConfig.DEFENSE_QUALITY_MIN) &&
    scoreAtLeast(scores.valuation_score, StrategyGateConfig.DEFENSE_VALUATION_MIN) &&
    scoreAtLeast(scores.size_score, StrategyGateConfig.DEFENSE_SIZE_MIN)
  );
}

function passesStableDefenseGate(scores: Record<string, number | undefined>): boolean {
  return (
    scoreAtLeast(scores.overall_score, StrategyGateConfig.DEFENSE_OVERALL_MIN) &&
    scoreAtLeast(scores.moat_score, StrategyGateConfig.STABLE_DEFENSE_MOAT_MIN) &&
    scoreAtLeast(scores.quality_score, StrategyGateConfig.STABLE_DEFENSE_QUALITY_MIN) &&
    scoreAtLeast(scores.valuation_score, StrategyGateConfig.STABLE_DEFENSE_VALUATION_MIN) &&
    scoreAtMost(scores.tactical_score, StrategyGateConfig.STABLE_DEFENSE_TACTICAL_MAX) &&
    scoreAtMost(scores.upside_score, StrategyGateConfig.STABLE_DEFENSE_UPSIDE_MAX)
  );
}

function hasSpeculationWeakness(scores: Record<string, number | undefined>): boolean {
  return (
    scoreAtMost(scores.overall_score, StrategyGateConfig.SPECULATION_OVERALL_MAX) ||
    scoreAtMost(scores.moat_score, StrategyGateConfig.SPECULATION_MOAT_MAX) ||
    scoreAtMost(scores.quality_score, StrategyGateConfig.SPECULATION_QUALITY_MAX) ||
    scoreAtMost(scores.valuation_score, StrategyGateConfig.SPECULATION_VALUATION_MAX)
  );
}

function gatedStrategyLabel(scores: Record<string, number | undefined>): string {
