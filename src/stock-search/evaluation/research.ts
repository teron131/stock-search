...
function createResearchAgents(systemPrompt: string, responseFormat: ZodType) {
  ...
  const saveResearchMemory = tool(
    async ({ ticker, notes }: { ticker: string; notes: string }, runtime: ToolRuntime) => {
      const key = researchMemoryKey(ticker);
      const store = (runtime.store as unknown as ResearchStore | null) ?? RESEARCH_STORE;
...
function buildResearchGraph(systemPrompt: string, responseFormat: ZodType) {
  ...
  async function loaderNode(state: ResearchGraphState) {
    ...
    const loadedNotes = (
      await Promise.all(
        urlBatches.map((batch) => agents.loader.invoke(`Load and summarize these URLs:\n${batch}`)),
      )
    ).join("\n\n");
    return { loadedNotes };
  }
  ...
  async function validatorNode(state: ResearchGraphState) {
    const evidence = [state.researchNotes, state.loadedNotes].filter(Boolean).join("\n\n");
