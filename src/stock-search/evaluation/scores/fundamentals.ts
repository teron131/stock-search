...
export function durableGrowthScore(indicator: IndicatorLike): number | null {
  const trendScore = weightedMeanScore([
    [statCurveScore(getNumberField(indicator, "revenue_growth_1y"), "revenue_growth"), 0.25],
    [statCurveScore(getNumberField(indicator, "revenue_cagr_3y"), "revenue_growth"), 0.35],
    [statCurveScore(getNumberField(indicator, "fcf_growth_1y"), "revenue_growth"), 0.1],
    [statCurveScore(getNumberField(indicator, "fcf_cagr_3y"), "revenue_growth"), 0.2],
    [marginStabilityScore(getNumberField(indicator, "operating_margin_std_3y")), 0.1],
  ]);
  if (trendScore != null) {
    return trendScore;
  }
  ...
  const marginScore = weightedMeanScore([
    [statCurveScore(getNumberField(indicator, "gross_margin"), "gross_margin"), 0.4],
    [statCurveScore(getNumberField(indicator, "operating_margin"), "operating_margin"), 0.6],
  ]);
  const fallbackScore = weightedMeanScore([
    [qualityGrowthScore(getNumberField(indicator, "revenue_growth"), "revenue_growth"), 0.6],
    [qualityGrowthScore(getNumberField(indicator, "eps_growth"), "eps_growth"), 0.25],
    [marginScore, 0.15],
  ]);
  ...
  if (
    ...
    calculatePeakCycleRisk(indicator) >= CycleNormalizationConfig.NO_TREND_DURABLE_GROWTH_CAP_RISK
  ) {
    return Math.min(fallbackScore, CycleNormalizationConfig.NO_TREND_DURABLE_GROWTH_CAP);
  }
...
export function marginPersistenceScore(indicator: IndicatorLike): number | null {
  ...
  return weightedMeanScore([
    [grossMarginMedian == null ? null : mapToCurveScore(grossMarginMedian, 20, 70, 45), 0.25],
    [
      operatingMarginMedian == null ? null : mapToCurveScore(operatingMarginMedian, 5, 45, 25),
      0.35,
    ],
    [fcfMarginMedian == null ? null : mapToCurveScore(fcfMarginMedian, -5, 35, 15), 0.25],
    [marginStabilityScore(getNumberField(indicator, "operating_margin_std_3y")), 0.15],
  ]);
}
...
function usesComparableFinancialsCurrency(indicator: IndicatorLike): boolean {
  ...
  return new Set(["USD", "EUR", "GBP", "CHF", "CAD", "AUD"]).has(currency.trim().toUpperCase());
}
...
function scalePersistenceScore(indicator: IndicatorLike): number | null {
  return weightedMeanScore([
    ...
    [scaleScore(getNumberField(indicator, "free_cash_flow"), "free_cash_flow"), 0.25],
    [statCurveScore(getNumberField(indicator, "revenue_cagr_3y"), "revenue_growth"), 0.25],
...
function capitalProductivityScore(indicator: IndicatorLike): number | null {
  ...
  const fcfMarginScore =
    fcfMarginMedian == null ? null : mapToCurveScore(fcfMarginMedian, -5, 35, 15);
...
function knowledgeCapitalScore(indicator: IndicatorLike): number | null {
  ...
  return productivelyGatedResearchScore(
    scaleScore(getNumberField(indicator, "rd_knowledge_capital"), "rd_knowledge_capital"),
    indicator,
  );
}
...
function rdProductivityScore(indicator: IndicatorLike): number | null {
  const rawScore = weightedMeanScore([
    ...
    [statCurveScore(getNumberField(indicator, "rd_intensity"), "rd_intensity"), 0.35],
  ]);
...
function structuralMoatProxyScore(indicator: IndicatorLike): number | null {
  return weightedMeanScore([
    [knowledgeCapitalScore(indicator), MoatSignalMultipliers.RD_KNOWLEDGE_CAPITAL],
    [rdProductivityScore(indicator), MoatSignalMultipliers.RD_PRODUCTIVITY],
    [marginPersistenceScore(indicator), MoatSignalMultipliers.MARGIN_PERSISTENCE],
    [
    ...
    [capitalProductivityScore(indicator), MoatSignalMultipliers.CAPITAL_PRODUCTIVITY],
  ]);
}

export function calculateMoatSignalScore(indicator: IndicatorLike): number | null {
  const economicScore = weightedMeanScore([
    ...
    [
      statCurveScore(getNumberField(indicator, "operating_margin"), "operating_margin"),
      MoatSignalMultipliers.OPERATING_MARGIN,
    ],
    [statCurveScore(getNumberField(indicator, "roe"), "roe"), MoatSignalMultipliers.ROE],
    [statCurveScore(getNumberField(indicator, "roic"), "roic"), MoatSignalMultipliers.ROIC],
    [
      statCurveScore(
        isBankLike(indicator) ? null : getNumberField(indicator, "debt_to_equity"),
        "debt_to_equity",
        true,
      ),
  ...
  if (!hasTrendHistory(indicator) && risk >= CycleNormalizationConfig.NO_TREND_MOAT_CAP_RISK) {
...
export function calculateQualitySignalScore(indicator: IndicatorLike): number | null {
  const factors: Array<[number | null, number]> = [
    ...
    [
      qualityGrowthScore(getNumberField(indicator, "revenue_growth"), "revenue_growth"),
    ...
    [statCurveScore(getNumberField(indicator, "roe"), "roe"), QualitySignalMultipliers.ROE],
    [statCurveScore(getNumberField(indicator, "roic"), "roic"), QualitySignalMultipliers.ROIC],
    [
      statCurveScore(getNumberField(indicator, "shareholder_yield"), "shareholder_yield"),
      QualitySignalMultipliers.SHAREHOLDER_YIELD,
    ],
  ];
  const availableFactorCount = factors.filter(([score]) => score != null).length;
  ...
  const rawScore = weightedMeanScore([
    ...
    [marginPersistenceScore(indicator), QualitySignalConfig.MARGIN_PERSISTENCE_WEIGHT],
    [
      fcfMarginMedian == null ? null : mapToCurveScore(fcfMarginMedian, -5, 35, 15),
    ...
    [
      weightedMeanScore([
        [marginStabilityScore(getNumberField(indicator, "operating_margin_std_3y")), 0.65],
        [
          beta == null
            ? null
  ...
  const cycleFloor =
    ...
    (currentQualityScore ?? 0) >= QualitySignalConfig.CYCLE_FLOOR_CURRENT_QUALITY_MIN &&
    (marginPersistenceScore(indicator) ?? 0) >=
      QualitySignalConfig.CYCLE_FLOOR_MARGIN_PERSISTENCE_MIN
      ? QualitySignalConfig.CYCLE_FLOOR
      : null;
  return Math.max(
    applyHighCycleCap(adjustedScore, cycleRisk, CycleNormalizationConfig.HIGH_RISK_QUALITY_CAP) ??
      adjustedScore,
    floor ?? adjustedScore,
