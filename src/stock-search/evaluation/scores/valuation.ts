...
function normalizedFcfYieldPercent(indicator: IndicatorLike): number | null {
  ...
  if (marketCap == null || marketCap <= 0 || revenue == null || fcfMarginMedian == null) {
    return null;
  }
  return ((revenue * fcfMarginMedian) / 100 / marketCap) * 100;
}

function fcfYieldSupportScore(indicator: IndicatorLike, anchors: ScoreAnchors): number | null {
  if (isBankLike(indicator)) {
    return null;
  }
  const fcfYieldRange = anchorRange("fcf_yield", anchors);
  return weightedMeanStatScore([
    [fcfYieldPercent(indicator), fcfYieldRange, FcfYieldBlendConfig.CURRENT_WEIGHT, false],
    [
...
function viableBusinessValuationFloor(
  ...
  if (
    ...
    (debtToEquity == null || debtToEquity <= anchorRange("debt_to_equity", anchors)[1]) &&
...
function forwardPeForValuation(indicator: IndicatorLike, anchors: ScoreAnchors): number | null {
  const forwardPeRange = anchorRange("pe_forward", anchors);
  const forwardPe = valuationMultipleField(indicator, "pe_forward", forwardPeRange);
...
function supportsQualityBackedValuationFloor(indicator: IndicatorLike): boolean {
...
function calculateLegacyValuationScore(indicator: IndicatorLike): number | null {
  ...
  const shareholderYieldRange = anchorRange("shareholder_yield", valuationAnchors);
  const operatingMarginRange = anchorRange("operating_margin", valuationAnchors);
  const roicRange = anchorRange("roic", valuationAnchors);
  const bankLike = isBankLike(indicator);

  const rawScore = weightedMeanStatScore([
    [valuationMultipleField(indicator, "peg", pegRange), pegRange, ValuationMultipliers.PEG, true],
    [valuationMultipleField(indicator, "pe", peRange), peRange, ValuationMultipliers.PE, true],
    [
    ...
    [getNumberField(indicator, "roic"), roicRange, ValuationMultipliers.ROIC, false],
    [marketCapScore(indicator), NORMALIZED_SCORE_RANGE, ValuationMultipliers.SIZE, false],
    [
  ...
  const adjustedScore = pullScoreTowardNeutral(rawScore, overheatRisk(indicator));
...
function calculateWarrantedFpeScore(indicator: IndicatorLike): number | null {
  ...
  const warrantedForwardPe =
    ...
    (1 + WarrantedFpeConfig.MOAT_SENSITIVITY * (scoreOrNeutral(moatScore) - 6)) *
    (1 + WarrantedFpeConfig.QUALITY_SENSITIVITY * (scoreOrNeutral(qualityScore) - 6)) *
    (1 + WarrantedFpeConfig.GROWTH_SENSITIVITY * (scoreOrNeutral(growthScore) - 5)) *
  ...
  return clampScore(
    DEFAULT_SCORE +
      WarrantedFpeConfig.RATIO_SCORE_SLOPE * Math.log2(warrantedForwardPe / actualForwardPe),
  );
}
...
export function calculateValuationScore(indicator: IndicatorLike): number | null {
  ...
  const guardedScore = applyCycleValuationGuard(flooredScore, indicator, cycleRisk);
