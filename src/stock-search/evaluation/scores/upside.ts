...
import { applyHighCycleCap, calculatePeakCycleRisk, overheatRisk } from "./cycle.js";
import { calculateMoatSignalScore, calculateQualitySignalScore } from "./fundamentals.js";
...
export function calculateCombinedUpsideScore(
  ...
  const epsGrowthScore = statCurveScore(getNumberField(indicator, "eps_growth"), "eps_growth");
  const analystTargetGapScore = statCurveScore(analystTargetGap, "median_upside");
  ...
  const availableRawCount = rawUpsideComponents.filter(([score]) => score != null).length;
  ...
  if (valuationScore != null && valuationScore < UpsideSupportConfig.LOW_VALUATION_TRUST) {
    cappedUpsideScore = Math.min(
      cappedUpsideScore,
  ...
  if (
    ...
    (calculateTacticalScore(indicator, analystTargetGap) ?? Number.POSITIVE_INFINITY) <
      UpsideSupportConfig.MATURE_TACTICAL_MAX
  ) {
    cappedUpsideScore = Math.min(cappedUpsideScore, UpsideSupportConfig.MATURE_UPSIDE_CAP);
  }
  if ((qualityScore != null && qualityScore < 3) || (moatScore != null && moatScore < 3)) {
    cappedUpsideScore = Math.min(cappedUpsideScore, WEAK_SUPPORT_UPSIDE_CAP);
  }
  if (valuationScore != null && valuationScore < 2 && qualityScore != null && qualityScore < 3) {
    cappedUpsideScore = Math.min(cappedUpsideScore, VERY_WEAK_SUPPORT_UPSIDE_CAP);
  }
  cappedUpsideScore -= overheatRisk(indicator) * OverheatSignalConfig.UPSIDE_PENALTY;
