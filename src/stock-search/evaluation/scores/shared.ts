...
import { getScoreAnchors, type ScoreAnchorKey, type ScoreAnchors } from "../anchors.js";
import {
  DEFAULT_SCORE,
  QualitySignalConfig,
  SCORE_SCALE,
...
export type WeightedFactorConfig = [number | null, [number, number, number], number, boolean];
export type IndicatorLike = Record<string, unknown>;

export const NORMALIZED_SCORE_RANGE: [number, number, number] = [0, SCORE_SCALE / 2, SCORE_SCALE];

export function getNumberField(indicator: IndicatorLike, fieldName: string): number | null {
...
export function qualityGrowthScore(
  ...
  return score == null ? null : Math.min(score, QualitySignalConfig.MAX_SINGLE_YEAR_GROWTH_SCORE);
}
...
export function weightedMeanUnit(factors: Array<[number | null, number]>): number {
  const availableFactors = factors.filter(
    (factor): factor is [number, number] => factor[0] != null,
  );
  const totalWeight = availableFactors.reduce((sum, [, weight]) => sum + weight, 0);
  ...
  return Math.max(
    ...
    Math.min(
      1,
      availableFactors.reduce((sum, [score, weight]) => sum + score * weight, 0) / totalWeight,
    ),
  );
}

export function weightedMeanStatScore(factors: WeightedFactorConfig[]): number | null {
...
export function weightedMeanScore(factors: Array<[number | null, number]>): number | null {
  const availableFactors = factors.filter(
    (factor): factor is [number, number] => factor[0] != null,
  );
  const totalWeight = availableFactors.reduce((sum, [, weight]) => sum + weight, 0);
...
export function isBankLike(indicator: IndicatorLike): boolean {
  const sector =
    typeof indicator.sector_name === "string" ? indicator.sector_name.toLowerCase() : "";
  const industry =
    typeof indicator.industry_name === "string" ? indicator.industry_name.toLowerCase() : "";
  return (
...
export function scaleScore(value: number | null, anchorKey: ScoreAnchorKey): number | null {
  if (value == null) {
    return null;
  }
  if (value <= 0) {
...
export function marketCapScore(
  ...
  const marketCap = "marketCap" in info ? asNumber(info.marketCap) : canonicalMarketCap(info);
