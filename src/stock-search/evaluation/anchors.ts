...
import { CalibrationConfig, MarketCapConfig, type MinMedMax } from "./constants.js";
...
export const STATIC_SCORE_ANCHORS: ScoreAnchors = {
  market_cap: [MarketCapConfig.MIN, MarketCapConfig.MEDIAN, MarketCapConfig.MAX],
...
export function calibrationDbPath(): string {
  return path.resolve(
    process.env.EVALUATION_CALIBRATION_SQLITE_PATH ?? DEFAULT_CALIBRATION_DB_PATH,
  );
}
...
function calibrationValue(row: CalibrationStatsRow, anchorKey: ScoreAnchorKey): number | null {
...
function dynamicAnchor(
  ...
  if (values.length < (options.minSampleSize ?? MIN_DYNAMIC_ANCHOR_SAMPLE_SIZE)) {
    return fallback;
  }
  const percentileSet = anchorPercentiles(anchorKey, options.sectorName);
  const anchors = percentileSet.map((percentileValue) => percentile(values, percentileValue));
  return [anchors[0] ?? fallback[0], anchors[1] ?? fallback[1], anchors[2] ?? fallback[2]];
}
...
function anchorPercentiles(
  ...
  if (ANCHOR_DIRECTIONS[anchorKey] === "inverse") {
    return sectorName == null ? INVERSE_PERCENTILES : SECTOR_INVERSE_PERCENTILES;
  }
  return sectorName == null ? POSITIVE_PERCENTILES : SECTOR_POSITIVE_PERCENTILES;
}

function sectorNameFromContext(context: AnchorContext): string | null {
  const sectorName = context?.sector_name ?? context?.sectorName;
  return typeof sectorName === "string" && sectorName.trim() ? sectorName.trim() : null;
}
...
function loadDynamicAnchors(): ScoreAnchors {
  ...
  return {
    ...
    shareholder_yield: dynamicAnchor(cachedCalibrationRows, "shareholder_yield"),
    rd_knowledge_capital: dynamicAnchor(cachedCalibrationRows, "rd_knowledge_capital"),
