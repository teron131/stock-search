...
function blendedResearchSignal({
  ...
  let score: number;
  if (researchScore != null && signalScore != null) {
    score = Number((researchWeight * researchScore + signalWeight * signalScore).toFixed(2));
...
export async function buildInputs(ticker: string): Promise<Evaluation> {
  ...
  const [outlook, research] = await Promise.all([
    runOptionalLlmEvaluation(ticker, FUTURE_OUTLOOK_DEFINITION, schemas.FutureOutlookSchema),
    runOptionalLlmEvaluation(ticker, RESEARCH_DEFINITION, schemas.ResearchEvaluationSchema),
  ]);
  ...
  const upsideScore = calculateCombinedUpsideScore(
    ...
    Array.isArray(indicator.ratings) ? (indicator.ratings as Array<Record<string, unknown>>) : null,
  );
...
export function evaluateAsset(inputs: Evaluation, ticker: string | null = null): EvaluationResult {
  const scores = {
