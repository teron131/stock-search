...
function randomInt(random: () => number, minValue: number, maxValue: number): number {
...
function randomSample<T>(random: () => number, values: T[], count: number): T[] {
  const pool = [...values];
  const picked: T[] = [];
...
function pickDemoHoldings(
  ...
  for (const [ticker, stockRow] of Object.entries(stocksMap)) {
    ...
    const strategy =
      typeof stockRow.indicators.strategy === "string" ? stockRow.indicators.strategy.trim() : null;
  ...
  const targetCount = Math.min(
    rankedPool.length,
    randomInt(random, DEMO_POSITION_COUNT_RANGE[0], DEMO_POSITION_COUNT_RANGE[1]),
  );
...
function buildDemoRows(
  ...
  for (const ticker of Object.keys(stocksMap).sort()) {
    ...
    rows.push({
      ...merged,
      ticker,
      name: typeof merged.name === "string" && merged.name.trim() ? merged.name : ticker,
      quantity,
      total,
  ...
  const totalValue = heldRows.reduce((sum, row) => sum + (safeFloat(row.total) ?? 0), 0);
  ...
  rows.sort(
    (left, right) => (safeFloat(right.weight_pct) ?? 0) - (safeFloat(left.weight_pct) ?? 0),
  );
  ...
  for (const row of heldRows) {
    ...
    const rowTotal = safeFloat(row.total) ?? 0;
    changeAbsolute += ((changePercent / 100) * rowTotal) / (1 + changePercent / 100);
  }
  ...
  return {
    ...
    portfolio_stats: {
      ...
      weighted_beta: weightedBeta == null ? null : Number(weightedBeta.toFixed(4)),
...
async function buildSectorsPayload(): Promise<Record<string, unknown>> {
  ...
  const sectorCount = new Set(
    sectors
      .map((sector) => (typeof sector.sector === "string" ? sector.sector.trim() : ""))
      .filter(Boolean),
  ).size;
  return {
    sectors,
