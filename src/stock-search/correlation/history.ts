...
import { dateFromKey, dateKey, emptyTimeSeriesFrame, frameFromRows } from "./time-series.js";
import type {
  CloseHistory,
  ClosePoint,
  TimeSeriesFrame,
...
export function resolveCorrelationTickers(tickers = DEFAULT_CORRELATION_TICKERS): string[] {
...
export async function fetchYahooCloseHistory(ticker: string): Promise<CloseHistory> {
...
export async function buildCloseRowsAndNames(
  ...
  for (let index = 0; index < tickers.length; index += MAX_FETCH_WORKERS) {
    histories.push(
      ...(await Promise.all(tickers.slice(index, index + MAX_FETCH_WORKERS).map(historyFetcher))),
    );
  }
