...
function presentTickerColumns(frame: TimeSeriesFrame, tickers: string[]): string[] {
...
export function selectTickerRows(frame: TimeSeriesFrame, tickers: string[]): TimeSeriesFrame {
...
export function pctChangeRows(frame: TimeSeriesFrame): TimeSeriesFrame {
  ...
  return frame
    ...
    .select(
      DATE_COLUMN,
      ...columns.map((column) => pl.col(column).div(pl.col(column).shift(1)).sub(1).alias(column)),
    )
...
export function sliceRowsToLookback(frame: TimeSeriesFrame, years: number): TimeSeriesFrame {
...
export function valuesForTicker(frame: TimeSeriesFrame, ticker: string): number[] {
  return rowsFromFrame(frame).map((row) => row.values.get(ticker) ?? Number.NaN);
}

export function pairCounts(frame: TimeSeriesFrame, tickers: string[]): number[][] {
  const result = Array.from({ length: tickers.length }, () => Array(tickers.length).fill(0));
...
export function correlationValues(frame: TimeSeriesFrame, tickers: string[]): number[][] {
  ...
  for (let leftIndex = 0; leftIndex < tickers.length; leftIndex += 1) {
    for (let rightIndex = leftIndex; rightIndex < tickers.length; rightIndex += 1) {
      ...
      const valid = frame.filter(pl.col(left).isFinite().and(pl.col(right).isFinite()));
      if (valid.height < 2) {
        continue;
      }
