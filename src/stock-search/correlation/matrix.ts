...
function matrixFromValues(tickers: string[], values: number[][]): CorrelationMatrix {
  return {
    tickers,
...
function valuesFromMatrix(matrix: CorrelationMatrix): number[][] {
  return matrix.values.map((row) => row.map((value) => (value == null ? Number.NaN : value)));
}
...
export function roundMatrix(matrix: CorrelationMatrix, decimals: number): CorrelationMatrix {
  const scale = 10 ** decimals;
  return {
    tickers: matrix.tickers,
    values: matrix.values.map((row) =>
      row.map((value) => (value == null ? null : Math.round(value * scale) / scale)),
    ),
  };
}
...
function calculateBlendedPairCorrelation(
  ...
  for (const input of inputs) {
    ...
    const clipped = Math.max(-1 + ATANH_EPSILON, Math.min(1 - ATANH_EPSILON, correlation));
...
function alignedMarketRows(
  ...
  const tickers = dedupePreserveOrder(rows.flatMap((row) => [...row.values.keys()])).filter(
    (ticker) => ticker !== marketTicker,
  );
  const result = [];
...
function residualizeReturns(frame: TimeSeriesFrame, marketTicker: string): TimeSeriesFrame {
  const rows = rowsFromFrame(frame);
  if (rows.length === 0) {
  ...
  for (const aligned of alignedMarketRows(frame, marketTicker)) {
    ...
    for (let index = 0; index < aligned.marketReturns.length; index += 1) {
      ...
      covariance += (aligned.tickerReturns[index] - tickerMean) * centeredMarket;
    }
    ...
    for (let index = 0; index < aligned.dates.length; index += 1) {
      const residual = aligned.tickerReturns[index] - (alpha + beta * aligned.marketReturns[index]);
...
export function buildBlendedMatrix(
  ...
  const symmetricValues = rawValues.map((row, rowIndex) =>
    row.map((value, columnIndex) => (value + rawValues[columnIndex][rowIndex]) / 2),
  );
  const shrunkValues = symmetricValues.map((row, rowIndex) =>
    row.map(
      (value, columnIndex) =>
        (1 - PSD_SHRINKAGE) * value + (rowIndex === columnIndex ? PSD_SHRINKAGE : 0),
    ),
  );
  ...
  const normalizedPsdValues = psdValues.map((row, rowIndex) =>
    row.map((value, columnIndex) =>
      Math.max(-1, Math.min(1, value / (scales[rowIndex] * scales[columnIndex]))),
    ),
  );
