...
function annualizedReturn(
	...
	const clean = rowsFromFrame(frame)
		...
		.filter((row): row is { date: Date; value: number } =>
...
export function buildTickerStats(
	...
	return tickers.map((ticker) => {
		...
		const monthlyStdDev = sampleStdDev(
			valuesForTicker(returns.monthly, ticker),
		);
		return {
			...
			annualizedStdDev:
				dailyStdDev == null
					? null
					: dailyStdDev * Math.sqrt(TRADING_DAYS_PER_YEAR),
		};
	});
}
...
export function buildSleeveWeightRecommendations({
	...
	for (const ticker of tickers) {
		...
		for (const marker of Array.isArray(markers) ? markers : []) {
			...
			membersByMarker.set(markerKey, [
				...(membersByMarker.get(markerKey) ?? []),
				ticker,
			]);
		}
	}
	...
	return [...membersByMarker.entries()]
		...
		.flatMap(([marker, members]) => {
			...
			const normalMeanPairCorrelation = meanPairCorrelation(
				normalCorrelationMatrix,
				uniqueMembers,
			);
			...
			const varianceRatio =
				(1 + (uniqueMembers.length - 1) * meanPair) / uniqueMembers.length;
			...
			const correlationBasis =
				tailMeanPairCorrelation != null &&
				tailMeanPairCorrelation >= normalMeanPairCorrelation
					? "tail_raw"
					: "normal";
			return [
				{
					...
					recommendedMemberWeightEqual:
						recommendedTotalWeight / uniqueMembers.length,
				},
			];
		});
}
