...
function normalizeSourceTickers(record: Record<string, unknown>): string[] {
  ...
  return Array.isArray(sourceTickers) ? sourceTickers.map((value) => String(value)) : [];
}
...
function normalizePortfolioNewsSummaryArticle(value: unknown): unknown {
  ...
  const relevancy =
    typeof record.relevancy === "string" && ARTICLE_RELEVANCIES.has(record.relevancy)
      ? record.relevancy
      : "low";
  const category =
    typeof record.category === "string" && ARTICLE_CATEGORIES.has(record.category)
      ? record.category
      : "other";
  const sentiment =
    typeof record.sentiment === "string" && ARTICLE_SENTIMENTS.has(record.sentiment)
      ? record.sentiment
      : "neutral";
...
const PortfolioNewsSummaryPayloadSchema = z
  .object({
    rows: z
      .array(z.preprocess(normalizePortfolioNewsSummaryRow, PortfolioNewsSummaryRequestRowSchema))
      .default([]),
    items: z
      .array(
        z.preprocess(
...
export async function loadPortfolioNews(
  ...
  const portfolioNewsRow = rows.find((row) => row.ticker === PORTFOLIO_NEWS_STORAGE_TICKER);
  if (!portfolioNewsRow) {
    return null;
  }
...
export async function savePortfolioNews(
  ...
  const portfolioNews = normalizePortfolioNewsForStorage(writePayload, currentPortfolioNews);
...
export async function savePortfolioNewsSummary(
  ...
  const portfolioNews = await normalizePortfolioNewsSummaryForStorage(store, summaryWrite);
  await store.saveNews(
    [portfolioNewsToRow(portfolioNews)],
    portfolioNewsStorageKey(portfolioNews.key),
  );
...
function portfolioNewsToRow(portfolioNews: PortfolioNewsPayload): CachedNewsRow {
...
async function normalizePortfolioNewsSummaryForStorage(
  ...
  return PortfolioNewsPayloadSchema.parse({
    ...
    as_of_date: asOfDate,
    window_start: input.window_start ?? currentPortfolioNews?.window_start ?? null,
    window_end: input.window_end ?? currentPortfolioNews?.window_end ?? asOfDate,
...
async function buildRawTickerNewsResult({
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "news fetch failed";
    return {
      group: {
        ticker,
        articles: [],
