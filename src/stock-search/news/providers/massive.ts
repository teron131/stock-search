...
const DEFAULT_MASSIVE_RESULTS = 50;

export async function getNewsMassiveAsync({
  ticker,
  nDays = 3,
  maxResults = DEFAULT_MASSIVE_RESULTS,
  client,
}: {
  ticker: string;
  nDays?: number;
  ...
  const boundedMaxResults = Math.min(maxResults, MASSIVE_PROVIDER_MAX_RESULTS);
  const params = {
  ...
  return (payload?.results ?? []).map((row) => {
    ...
    const sentiment =
      SENTIMENT_MAP[String((insights[0] as Record<string, unknown>).sentiment ?? "")] ?? "neutral";
