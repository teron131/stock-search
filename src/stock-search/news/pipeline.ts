...
export { buildNewsTickerIdentity, type NewsTickerIdentity } from "./pipeline/identity.js";
...
import { type PortfolioSummaryDeps, summarizePortfolioNews } from "./pipeline/summarization.js";
...
export const newsRuntime = {
  analyzeNews: (ticker: string, newsList: NewsArticle[], tickerIdentity?: NewsTickerIdentity) =>
...
async function loadChatOpenAI(): Promise<AnalysisDeps["chatOpenAI"] | undefined> {
  try {
...
function sortNewsCandidatesByMetadata(
  ...
  return rankNewsCandidates(tickerIdentity, rawNewsList).slice(0, candidateCount);
}
...
async function labelNewsWithLlm(
  ...
  try {
    return await newsRuntime.analyzeNews(context.ticker, newsList, context.tickerIdentity);
  } catch {
...
async function attachRawFastContentExcerpts(newsList: NewsArticle[]): Promise<NewsArticle[]> {
  ...
  return newsList.map((news, index) => {
    ...
    const contentExcerpt =
      typeof content === "string" && content.trim()
        ? trimRawFastNewsText(normalizeRawFastNewsText(content), MAX_RAW_FAST_NEWS_CONTENT_CHARS)
        : null;
