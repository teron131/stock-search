...
type ChatOpenAIClient = (input: { model: string; temperature: number; reasoningEffort: "low" }) => {
...
function normalizeSummaryTickers(tickers: string[], allowedTickers: Set<string>): string[] {
...
function cleanPortfolioNewsSummaryChapters(
  ...
  for (const chapter of chapters) {
    ...
    const tickers = normalizeSummaryTickers(parsedChapter.tickers, allowedTickers);
...
function fallbackTickerChapters({
  ...
  const tickerItems = normalizedItems.filter((item) => item.source_tickers.includes(ticker));
...
function fallbackMacroChapters({
  ...
  normalizedItems.forEach((item, index) => {
    ...
    const sourceTickers = item.source_tickers.filter((ticker) => heldTickers.has(ticker));
  ...
  return macroCandidates.slice(0, MAX_PORTFOLIO_SUMMARY_MACRO_ITEMS).map((candidate) => ({
...
function buildTopTickerSummary({
  ...
  let chapters = cleanPortfolioNewsSummaryChapters(summaryEntry?.chapters ?? [], {
...
export async function summarizePortfolioNews({
  ...
  const normalizedItems = normalizePortfolioNewsSummaryItems(items, heldTickers);
