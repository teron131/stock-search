...
export function normalizeNewsUrl(rawUrl: string): string {
  try {
    ...
    return `${url.protocol}//${url.host.toLowerCase()}${normalizedPath}${query ? `?${query}` : ""}`;
  } catch {
...
function newsCandidateSignals(
  ...
  const terms =
    tickerIdentity.searchTerms.length > 0 ? tickerIdentity.searchTerms : [tickerIdentity.ticker];
  const title = news.title ?? "";
  const summary = news.summary ?? "";
  const url = news.url ?? "";
  const isFallback = FALLBACK_SUMMARIES.some((prefix) => summary.startsWith(prefix));
  return {
  ...
  return newsList
    ...
    .sort((left, right) => {
      ...
      if (left.signals.isConsentUrl !== right.signals.isConsentUrl) {
        return Number(left.signals.isConsentUrl) - Number(right.signals.isConsentUrl);
      }
...
export function balanceDomains(items: NewsArticle[]): NewsArticle[] {
  ...
  const domains = items.map((item) => (item.url ? extractDomain(item.url) : "")).filter(Boolean);
  if (domains.length === 0) {
    return items;
  }
...
export function finalizeNewsFeed(
  ...
  return filteredNewsList.sort((left, right) => {
    ...
    const rightDaysAgo =
      typeof right.days_ago === "number" ? right.days_ago : Number.POSITIVE_INFINITY;
