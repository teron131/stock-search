...
export class ProviderRequestLimiter {
  ...
  acquire(now = new Date()): boolean {
    ...
    this.requestTimes = this.requestTimes.filter((requestTime) => requestTime.getTime() > cutoff);
...
export type HttpClient = {
  get(input: { url: string; params: Record<string, string> }): Promise<HttpResponse>;
  post(input: {
    url: string;
...
async function fetchProviderBatch(providerSpecs: readonly ProviderSpec[]): Promise<NewsArticle[]> {
...
function providerQueryForTickerIdentity(tickerIdentity: NewsTickerIdentity): string {
  return tickerIdentity.companyName ? tickerIdentity.label : tickerIdentity.ticker;
}
...
export async function fetchRawNewsFromSources(
  ...
  if (rawNewsList.length < context.maxResults && hasEnvValue(process.env.EXA_API_KEY)) {
