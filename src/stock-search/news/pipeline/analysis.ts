...
function providerSummaryContent(news: NewsArticle): string | null {
  ...
  if (!summary || FALLBACK_SUMMARIES.some((prefix) => summary.startsWith(prefix))) {
    return null;
  }
...
async function buildAnalysisBatch(
  ...
  if (uncachedItems.length > 0) {
    const contentList = await deps.webloader(uncachedItems.map((item) => item.news.url));
  ...
  const readableItems = uncachedItems
    .map((item) => ({
      ...item,
      content: webContentByIndex.get(item.index) ?? providerSummaryContent(item.news),
    }))
    .filter(
...
export function fallbackAnalysisFromProviders(newsList: NewsArticle[]): NewsAnalysis[] {
...
function fillUncachedAnalysisFromProviders(
  ...
  const fallbacks = fallbackAnalysisFromProviders(uncachedItems.map((item) => item.news));
  uncachedItems.forEach((item, index) => {
...
export async function analyzeNews({
  ...
  const { results, uncachedItems } = splitCachedAnalysis(newsList, analysisCache);
  if (uncachedItems.length === 0) {
    return results;
  }
  ...
  const { readableItems, prompts } = await buildAnalysisBatch(tickerIdentity, uncachedItems, deps);
