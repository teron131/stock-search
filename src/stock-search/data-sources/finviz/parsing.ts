...
export function parseFinvizNumber(
	...
	const sign =
		firstToken.trim().startsWith("(") && firstToken.trim().endsWith(")")
			? -1
			: 1;
	return sign * parsed * multiplier;
}

function parseSecondFinvizNumber(
	value: string | null | undefined,
): number | null {
	if (value == null) {
		return null;
...
export function parseFinvizQuoteSnapshot(
	...
	return {
		...
		eps_past_3y_growth: pickNumber(
			raw,
			FINVIZ_LABELS.epsPastThreeFiveYearGrowth,
		),
		eps_past_5y_growth: parseSecondFinvizNumber(
			raw[FINVIZ_LABELS.epsPastThreeFiveYearGrowth],
		),
		sales_past_3y_growth: pickNumber(
			raw,
			FINVIZ_LABELS.salesPastThreeFiveYearGrowth,
		),
		sales_past_5y_growth: parseSecondFinvizNumber(
			raw[FINVIZ_LABELS.salesPastThreeFiveYearGrowth],
		),
