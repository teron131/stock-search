...
function randomBatchGapMs(): number {
  return (
    ...
    Math.floor(Math.random() * (FINVIZ_BATCH_GAP_MAX_MS - FINVIZ_BATCH_GAP_MIN_MS + 1))
  );
}
...
export class FinvizSource {
  ...
  async getQuoteSnapshot(): Promise<FinvizQuoteSnapshot> {
    this.snapshotPromise ??= finvizRequestQueue.schedule(() => this.fetchQuoteSnapshot());
  ...
  private async fetchQuoteSnapshot(): Promise<FinvizQuoteSnapshot> {
    ...
    const snapshot = parseFinvizQuoteSnapshot(html, this.ticker, url, new Date().toISOString());
