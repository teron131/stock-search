...
type YahooQuoteSummaryModule = Record<string, YahooQuoteSummaryField | string | undefined>;
...
const YAHOO_SEARCH_URL = "https://query2.finance.yahoo.com/v1/finance/search?q={ticker}";
const YAHOO_FX_URL =
...
const fxRateCache = new Map<string, { fetchedAtMs: number; rate: number | null }>();
...
function computeBeta(series: DatedClose[], benchmarkSeries: DatedClose[]): number | null {
  ...
  const stockMean = aligned.reduce((sum, [stockReturn]) => sum + stockReturn, 0) / aligned.length;
  const benchmarkMean =
    aligned.reduce((sum, [, benchmarkReturn]) => sum + benchmarkReturn, 0) / aligned.length;
  let covariance = 0;
...
async function fetchCurrencyToUsdRate(currencyInput: unknown): Promise<number | null> {
  ...
  const rate = toFiniteNumber(result?.meta?.regularMarketPrice) ?? toFiniteNumber(lastClose);
...
function buildSessionMarketData(
  ...
  const baseline =
    quoteSummaryNumberField([priceModule], "regularMarketPreviousClose") ?? fallbackPreviousClose;

  return {
    currentPrice,
    change:
      currentPrice != null && baseline != null ? roundOptional(currentPrice - baseline) : null,
    changePercent1d:
...
function buildRatingsSnapshot(
  ...
  const history =
    quoteSummaryPayload?.quoteSummary?.result?.[0]?.upgradeDowngradeHistory?.history ?? [];
  ...
  return {
    ...
    ratings: sortedHistory
      .map((entry) => {
        ...
        const firm = typeof entry.firm === "string" && entry.firm.trim() ? entry.firm.trim() : null;
        const toGrade =
          typeof entry.toGrade === "string" && entry.toGrade.trim() ? entry.toGrade.trim() : null;
        const fromGrade =
          typeof entry.fromGrade === "string" && entry.fromGrade.trim()
            ? entry.fromGrade.trim()
            : null;
        const action =
          typeof entry.action === "string" && entry.action.trim() ? entry.action.trim() : null;
        const priceTargetAction =
          typeof entry.priceTargetAction === "string" && entry.priceTargetAction.trim()
        ...
        return {
          date: epochGradeDate != null ? new Date(epochGradeDate * 1000).toISOString() : null,
...
async function fetchYahooQuoteSummary(ticker: string): Promise<YahooQuoteSummaryResponse | null> {
  try {
    ...
    const quoteSummaryResponse = await fetch(buildYahooQuoteSummaryUrl(ticker, crumb), {
      headers: {
        "user-agent": "Mozilla/5.0",
        cookie: mergeCookieHeaders(cookieHeaders, getSetCookieHeaders(crumbResponse.headers)),
      },
    });
...
function pointsForType(
  ...
  return Array.isArray(values)
    ? values.filter(
        (value): value is YahooFundamentalsPoint => typeof value === "object" && value !== null,
      )
    : [];
}
...
function sumTrailingValues(points: YahooFundamentalsPoint[], count: number): number | null {
...
function computeMarginPercent(
  ...
  if (annualNumerator == null || annualDenominator == null || annualDenominator === 0) {
    return null;
  }
...
function buildFundamentalsSnapshot(
  ...
  const quarterlyOperatingIncome = pointsForType(payload, "quarterlyOperatingIncome");
  ...
  const dilutedShares = latestPointValue(quarterlyShares) ?? latestPointValue(annualShares);
  ...
  const forwardEps = quoteSummaryNumberField([defaultKeyStatisticsModule], "forwardEps");
  ...
  return {
    ...
    pe_forward:
      roundOptional(
        quoteSummaryNumberField([summaryDetailModule, defaultKeyStatisticsModule], "forwardPE"),
        2,
      ) ??
    ...
    beta:
      roundOptional(
        quoteSummaryNumberField([summaryDetailModule, defaultKeyStatisticsModule], "beta"),
        2,
      ) ??
      roundOptional(quoteSummaryNumberField([defaultKeyStatisticsModule], "beta3Year"), 2) ??
...
export class YahooFinanceSource {
  ...
  async getIndicatorsSnapshot(): Promise<Record<string, unknown>> {
    ...
    const [payload, intradayPayload, fundamentalsPayload, quoteSummaryPayload, benchmarkPayload] =
      await Promise.all([
        ...
        fetchJson<YahooChartResponse>(
          YAHOO_CHART_URL.replace("{ticker}", encodeURIComponent(YAHOO_BENCHMARK_TICKER)),
        ),
      ]);
    const result = payload?.chart?.result?.[0];
    const intradayResult = intradayPayload?.chart?.result?.[0];
    const meta = result?.meta ?? {};
    const series = buildSeries(result?.timestamp, result?.indicators?.quote?.[0]?.close);
    ...
    return {
      ...
      change_percent_1m:
        currentPrice != null
          ? percentChange(
              currentPrice,
              latest ? lastCloseAtOrBefore(series, subtractMonths(latest.date, 1)) : null,
            )
          : null,
      change_percent_3m:
        currentPrice != null
          ? percentChange(
              currentPrice,
              latest ? lastCloseAtOrBefore(series, subtractMonths(latest.date, 3)) : null,
            )
          : null,
      change_percent_6m:
        currentPrice != null
          ? percentChange(
              currentPrice,
              latest ? lastCloseAtOrBefore(series, subtractMonths(latest.date, 6)) : null,
            )
          : null,
      change_percent_1y:
        currentPrice != null
          ? percentChange(
              currentPrice,
              latest ? lastCloseAtOrBefore(series, subtractMonths(latest.date, 12)) : null,
            )
          : null,
      iv: computeIv(series),
