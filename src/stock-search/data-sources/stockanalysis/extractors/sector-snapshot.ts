/** Sector summary extraction and top-ticker cache enrichment. */
...
import { isCacheTimestampFresh, loadJsonCache, writeJsonCache } from "../../../cache.js";
import type { StockAnalysisSectorSnapshot, StockAnalysisSectorSummary } from "../schemas.js";
import { DEFAULT_CONTENT_OPTIONS, loadStockAnalysisPage } from "./exa-client.js";
import { STOCKANALYSIS_SECTOR_URL, STOCKANALYSIS_SECTORS_URL } from "./urls.js";
...
const SectorRowSchema = z
  .object({
    ...
    change_percent_1d: NullableNumber.describe("Sector one-day change percent."),
    change_percent_1y: NullableNumber.describe("Sector one-year change percent."),
  })
  .describe("One StockAnalysis sector snapshot row.");

const SectorSnapshotSchema = z
  .object({
    sectors: z.array(SectorRowSchema).default([]).describe("Current sector snapshot rows."),
  })
...
const SectorSnapshotCacheSchema = z
  .object({
    ...
    sectors: z.array(SectorRowSchema).default([]).describe("Cached sector snapshot rows."),
  })
  .describe("Cached StockAnalysis sector snapshot.");

const SectorTopTickerCacheEntrySchema = z
  .object({
    fetched_at: z.string().describe("Timestamp when the sector top-ticker list was fetched."),
    tickers: z.array(z.string()).default([]).describe("Representative top tickers for the sector."),
  })
...
function parseSectorTopTickers(text: string): string[] {
  ...
  for (const line of text.split("\n")) {
    const rowMatch = line.trim().match(/^\|\s*\d+\s*\|\s*([A-Za-z][A-Za-z0-9.-]*)\s*\|/);
    if (!rowMatch?.[1]) {
      continue;
    }
...
function normalizeSectorRows(rows: SectorRow[]): StockAnalysisSectorSummary[] {
  return rows
    ...
    .filter(
      (row): row is StockAnalysisSectorSummary => !!row.sector && Number.isFinite(row.stock_count),
    );
}
...
function readCachedSectorTopTickers(
  ...
  const isFresh = isCacheTimestampFresh(entry.fetched_at, now, SECTOR_TOP_TICKER_CACHE_TTL_MS);
  return {
...
async function fetchSectorTopTickers(sectorNames: string[]): Promise<Map<string, string[]>> {
  ...
  const urls = sectorNames.map((sectorName) =>
    STOCKANALYSIS_SECTOR_URL.replace("{sector}", sectorSlug(sectorName)),
  );
  const { pages } = await agent.load(urls);
...
async function loadSectorRowsWithCache(): Promise<StockAnalysisSectorSummary[]> {
  const now = new Date();
  const cache = await loadJsonCache(SECTOR_SNAPSHOT_CACHE_PATH, SectorSnapshotCacheSchema, () => ({
    fetched_at: null,
    sectors: [],
  }));
  ...
  try {
    ...
    if (fetchedRows.length > 0) {
      await writeJsonCache(SECTOR_SNAPSHOT_CACHE_PATH, {
