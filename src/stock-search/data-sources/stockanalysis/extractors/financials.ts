...
import { loadStockAnalysisPageOrDefault, loadStockAnalysisText } from "./exa-client.js";
...
const FinancialsSchema = z
  .object({
    ...
    revenue_growth: NullableNumber.optional().describe(getFieldDescription("revenue_growth")),
    ...
    gross_margin: NullableNumber.optional().describe(getFieldDescription("gross_margin")),
    operating_margin: NullableNumber.optional().describe(getFieldDescription("operating_margin")),
    ...
    financials_currency: z
      ...
      .nullable()
      .optional()
      .describe("Currency code shown in the StockAnalysis financials unit label."),
  ...
  .describe("Income statement fields extracted from the StockAnalysis financials page.");
...
function financialsFieldMultiplier(fieldName: FinancialsFieldName, unitMultiplier: number): number {
...
function financialsRowLabels(labelLines: string[]): string[] {
  ...
  for (const line of labelLines) {
    if (line === "Revenue Growth (YoY)" && labels[labels.length - 1] !== "Revenue") {
      labels.push("Revenue");
    }
    labels.push(line);
  }
...
function parseFinancialsTable(text: string): FinancialsTable | null {
  ...
  if (periodEndingIndex < 0 || tableHeaderIndex < 0 || tableHeaderIndex <= periodEndingIndex) {
    return null;
  }

  const rowLabels = financialsRowLabels(lines.slice(periodEndingIndex + 1, tableHeaderIndex));
  const tableRows = lines.slice(tableHeaderIndex).filter((line) => line.startsWith("|"));
...
function financialsTableValue(
  ...
  return value == null
    ? null
    : parseStockAnalysisNumber(value, financialsFieldMultiplier(fieldName, table.unitMultiplier));
}

function percentChange(currentValue: number | null, priorValue: number | null): number | null {
  if (
    currentValue == null ||
    priorValue == null ||
    priorValue === 0 ||
...
function deriveFinancialTrendFields(table: FinancialsTable): Record<string, unknown> {
  ...
  const tableValue = (
    ...
    columnIndex == null ? null : financialsTableValue(table, label, columnIndex, fieldName);

  const latestRevenue = tableValue("Revenue", latestIndex, "revenue");
  const priorRevenue = tableValue("Revenue", priorIndex, "revenue");
  const threeYearBaseRevenue = tableValue("Revenue", threeYearBaseIndex, "revenue");
  ...
  const threeYearBaseFcf = tableValue("Free Cash Flow", threeYearBaseIndex, "free_cash_flow");
  const latestRd = tableValue("Research & Development", latestIndex, "research_and_development");
  const rdValues = fiscalColumns
    .slice(0, 4)
    .map((columnIndex) =>
      tableValue("Research & Development", columnIndex, "research_and_development"),
    );
  ...
  const sharesChange = tableValue("Shares Change (YoY)", latestIndex, "shares_change_1y");
  ...
  const rdKnowledgeCapital = rdValues.reduce<number | null>(
    (sum, value, index) =>
      value == null ? sum : (sum ?? 0) + value * (knowledgeCapitalWeights[index] ?? 0),
    null,
  );

  return {
    financials_currency: table.currency,
    revenue_growth_1y: roundTrendValue(percentChange(latestRevenue, priorRevenue)),
    revenue_cagr_3y: roundTrendValue(cagrPercent(latestRevenue, threeYearBaseRevenue, 3)),
    ...
    operating_margin_std_3y: roundTrendValue(standardDeviation(operatingMargins)),
...
function parseFinancialsSnapshotFromText(text: string): Record<string, unknown> {
...
export async function loadFinancialsSnapshot(
  ...
  const parsedFinancials =
    financialsText == null ? {} : parseFinancialsSnapshotFromText(financialsText);
