...
import { STOCKANALYSIS_OVERVIEW_URL, STOCKANALYSIS_STATISTICS_URL, stockDataUrl } from "./urls.js";
...
const QuoteFieldsSchema = z
  ...
  .describe("Quote header fields extracted from the StockAnalysis overview page.");
...
const RatingRowSchema = z
  .object({
    ...
    from_grade: z
      ...
      .nullable()
      .optional()
      .describe("Previous analyst rating grade, if a rating table provides one."),
    action: z
      .string()
      .nullable()
      .optional()
  ...
  .describe("Analyst forecast row extracted from the StockAnalysis statistics page.");

const StatisticsSchema = z
  .object({
    market_cap: NullableNumber.optional().describe(getFieldDescription("market_cap")),
    ...
    pe_forward: NullableNumber.optional().describe(getFieldDescription("pe_forward")),
    ps: NullableNumber.optional().describe(getFieldDescription("ps")),
    ps_forward: NullableNumber.optional().describe(getFieldDescription("ps_forward")),
    ...
    debt_to_equity: NullableNumber.optional().describe(getFieldDescription("debt_to_equity")),
    debt_to_ebitda: NullableNumber.optional().describe(
      "Debt / EBITDA value from the Financial Position section.",
    ),
    free_cash_flow: NullableNumber.optional().describe(getFieldDescription("free_cash_flow")),
    shareholder_yield: NullableNumber.optional().describe(getFieldDescription("shareholder_yield")),
    median_upside: NullableNumber.optional().describe(getFieldDescription("median_upside")),
    ratings: z
...
function parseStatisticLine(line: string): [StatisticsFieldName, number] | null {
  ...
  if (trimmedLine.startsWith("|")) {
    ...
    return fieldName != null && parsedValue != null ? [fieldName, parsedValue] : null;
  }
...
function parseStatisticsSnapshotFromText(text: string): Record<string, unknown> {
...
function hasParsedStatisticsFields(fields: Record<string, unknown>): boolean {
  return PARSED_STATISTICS_REQUIRED_FIELDS.some((fieldName) => fields[fieldName] != null);
}

/** Load quote fields from a StockAnalysis overview page. */
export async function loadQuoteFields(tickerLower: string): Promise<Required<QuoteFields>> {
...
export async function loadStatisticsSnapshot(
  ...
  const parsedStatistics =
    statisticsText == null ? {} : parseStatisticsSnapshotFromText(statisticsText);
