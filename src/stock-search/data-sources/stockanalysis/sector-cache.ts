...
export type SectorSnapshotCacheStore = {
  ...
  saveSectorSnapshot(snapshot: StockAnalysisSectorSnapshot, key?: string): Promise<void>;
};
...
export function normalizeSectorSnapshot(value: unknown): StockAnalysisSectorSnapshot | null {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return null;
  }
...
export function isFreshSectorSnapshot(
  ...
  return (
    ...
    isCacheTimestampFresh(snapshot.meta.fetched_at, Date.now(), SECTOR_SNAPSHOT_CACHE_TTL_MS)
  );
}
