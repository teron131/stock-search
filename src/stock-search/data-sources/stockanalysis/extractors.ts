...
function financialsFiscalColumnIndexes(header: string[]): number[] {
	return header
...
function deriveFinancialTrendFields(
	...
	const fiscalColumns = financialsFiscalColumnIndexes(table.header);
	...
	const latestRevenue = financialsTableValue(
		table,
		"Revenue",
		latestIndex,
		"revenue",
	...
	const priorRevenue =
		priorIndex == null
			? null
			: financialsTableValue(table, "Revenue", priorIndex, "revenue");
	const threeYearBaseRevenue =
		threeYearBaseIndex == null
			? null
			: financialsTableValue(table, "Revenue", threeYearBaseIndex, "revenue");
	const latestFcf = financialsTableValue(
		table,
		"Free Cash Flow",
		latestIndex,
		"free_cash_flow",
	...
	const priorFcf =
		priorIndex == null
			? null
			: financialsTableValue(
					table,
					"Free Cash Flow",
					priorIndex,
					"free_cash_flow",
				);
	const threeYearBaseFcf =
		threeYearBaseIndex == null
			? null
			: financialsTableValue(
					table,
	...
	const latestRd = financialsTableValue(
		table,
	...
	const grossMargins = finiteValues(
		marginColumnIndexes.map((columnIndex) =>
			financialsTableValue(table, "Gross Margin", columnIndex, "gross_margin"),
		),
	);
	...
	const sharesChange = financialsTableValue(
		table,
		"Shares Change (YoY)",
		latestIndex,
		"shares_change_1y",
	);
	const latestSharesOutstanding = financialsTableValue(
		table,
		"Shares Outstanding",
		latestIndex,
		"shares_change_cagr_3y",
	);
	const threeYearBaseSharesOutstanding =
		threeYearBaseIndex == null
			? null
			: financialsTableValue(
					table,
