...
export function formatLastUpdatedText(timestamp, { isUsingDemoData = false } = {}) {
...
export function syncLogoutButton(authSession) {
  ...
  const shouldShow =
    !CONFIG.isDemoMode && Boolean(authSession?.enabled) && Boolean(authSession?.authenticated);

  logoutBtn.hidden = !shouldShow;
  const tooltip = shouldShow && authSession?.email ? `Logout (${authSession.email})` : "Logout";
...
export function syncRefreshButtonState({
  ...
  refreshBtn.setAttribute("aria-pressed", isDashboardSyncing ? "true" : "false");
}
...
export function initSidebarAndNav() {
  ...
  if (window.innerWidth <= 1024) {
    ...
    if (topBarLeft) {
      ...
      cleanupFns.push(() => topBarLeft.removeEventListener("click", toggleSidebar));
    }
  }
