...
const TICKER_TAPE_PLACEHOLDER_HTML = '<div class="tradingview-widget-container__widget"></div>';
...
const TRADING_VIEW_PLACEHOLDER_HTML = '<div class="tradingview-widget-container__widget"></div>';
const HEATMAP_SCRIPT_SRC =
  "https://s3.tradingview.com/external-embedding/embed-widget-stock-heatmap.js";
const CALENDAR_SCRIPT_SRC = "https://s3.tradingview.com/external-embedding/embed-widget-events.js";
const DEFAULT_HEATMAP_SOURCE = "SPX500";

let tickerTapeRetryId = null;

function hasLoadedTickerTape(container, symbolsKey) {
  return container.dataset.symbols === symbolsKey && Boolean(container.querySelector("iframe"));
}

function shouldRetryTickerTape(container, symbolsKey) {
  return container.dataset.symbols === symbolsKey && !container.querySelector("iframe");
}
  ...
  tabs.forEach((tab) => {
    const onClick = () => {
      ...
      tabs.forEach((tabButton) => {
        tabButton.classList.toggle("active", tabButton.dataset.source === source);
      });
