...
import { buildCacheSnapshot, isCacheFresh, writeTickerNewsCache } from "./cache.js";
import { mapWithConcurrency, mergeNewsItems, pruneRetainedNewsItems } from "./dataModel.js";
...
async function loadDemoNewsFeed(heldTickers) {
  ...
  return {
    items: mergeNewsItems(
      heldTickers.flatMap((ticker) => normalizedPayload.items_by_ticker[ticker] || []),
    ),
    generatedAt: normalizedPayload.meta.generated_at || new Date().toISOString(),
    failedTickers: [],
  };
}
...
async function loadLiveNewsFeed({ heldTickers, force = false, staleTickers = [] }) {
  ...
  const results = await mapWithConcurrency(
    ...
    async (ticker) => {
      try {
        ...
        const items = pruneRetainedNewsItems(normalizeTickerNewsPayload(payload, ticker));
  ...
  const failedTickers = results.filter((result) => !result.ok).map((result) => result.ticker);

  return {
    items: mergeNewsItems(Array.from(nextTickerItems.values()).flatMap((items) => items || [])),
    generatedAt:
      results.find((result) => result.ok)?.fetchedAt || cacheSnapshot.generatedAt || null,
    failedTickers,
    cacheSnapshot,
  };
}
...
export function planNewsFeedLoad({
  ...
  const shouldLoadInBackground = background || visibleItemCount > 0 || hasCachedItems;
