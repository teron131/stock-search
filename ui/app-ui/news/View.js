...
function normalizedList(values, fallbackValue) {
  const sourceValues = Array.isArray(values) ? values : fallbackValue ? [fallbackValue] : [];
  return Array.from(
    new Set(sourceValues.map((value) => String(value || "").trim()).filter(Boolean)),
  );
}
...
function buildNewsSignals({ items, heldTickers }) {
  ...
  for (const item of items) {
    const facets = collectStoryFacets(item);
    for (const ticker of normalizedList(item.sourceTickers, item.sourceTicker)) {
    ...
    const isRecentByTimestamp = timestamp > 0 && Date.now() - timestamp <= 2 * 24 * 60 * 60 * 1000;
  ...
  const coveragePct =
    heldTickers.length > 0 ? Math.round((coveredTickers.size / heldTickers.length) * 100) : 0;
  const recentStoryPct = storyCount > 0 ? Math.round((recentStoryCount / storyCount) * 100) : 0;
  const bullishPct = sentimentCount > 0 ? Math.round((bullishCount / sentimentCount) * 100) : 0;
  const bearishPct = sentimentCount > 0 ? Math.round((bearishCount / sentimentCount) * 100) : 0;
...
function renderPulseScale({ label, value, tone, caption }) {
  ...
  const displayValue = tone === "mixed" && value > 0 ? `+${value}%` : `${value}%`;
...
function renderHighlightedText(text, tickers) {
  ...
  const normalizedTickers = Array.from(
    new Set((tickers || []).map((ticker) => String(ticker || "").trim()).filter(Boolean)),
...
function renderSummaryChapters(chapters) {
  ...
  return html`
    <div className="news-summary-list">
      ${safeChapters.map(
        (chapter, index) => html`
          <div key=${`${chapter.headline || "chapter"}-${index}`} className="news-summary-item">
            <div className="news-summary-item-topic">${chapter.headline}</div>
            <div className="news-summary-item-text">
              ${renderHighlightedText(chapter.paragraph, chapter.relatedTickers)}
            </div>
          </div>
        `,
      )}
...
function renderFacetTags(item, { compact = false } = {}) {
  ...
  return tags.map(
    (tag) => html`<span key=${tag.key} className=${tag.className}>${tag.label}</span>`,
  );
}
...
export function NewsView({
  ...
  const newsSignals = useMemo(() => buildNewsSignals({ items, heldTickers }), [heldTickers, items]);
  const showFeedLoadingState = !isWaitingOnPortfolio && hasHoldings && !hasItems && isLoading;
        ...
        <div className="news-toolbar-copy">
          ...
          <div className="news-toolbar-heading">
            ...
            <div className="news-toolbar-status">
              ${coverageText}
              ${isRefreshing
                ? html` <span className="news-toolbar-note">Background refresh</span> `
                : null}
          ...
          ${portfolioNewsSummary?.hasNews
                  ...
                  <div className="news-summary-section is-top-tickers">
                    ...
                    <div className="news-ticker-briefs">
                      ${portfolioNewsSummary.topTickers.map((summaryItem) => {
                        const tickerFacets = tickerFacetMap.get(summaryItem.ticker);
                        const weightLabel =
                          summaryItem.weightLabel || formatWeight(summaryItem.weightPct);
                        return html`
                          <article key=${summaryItem.ticker} className="news-ticker-brief">
                            <div className="news-ticker-brief-header">
                              <div className="news-ticker-brief-ticker">${summaryItem.ticker}</div>
                              ${weightLabel !== "--"
                                ? html`
                                    <div className="news-ticker-brief-weight">${weightLabel}</div>
                                  `
                                : null}
                            </div>
                            ${tickerFacets
            ...
            : html`
                <div className="news-summary-body news-summary-placeholder">
                  ...
                  <div className="news-summary-copy">${summaryPlaceholderCopy}</div>
                </div>
              `}
        </div>
            ...
            ? html`
                <div className="news-list">
                  ${items.map((item) => {
                    ...
                    const summary = item.summary || "No article note available.";
                    const canToggleSummary = String(summary || "").trim().length > 160;
                    ...
                    return html`
                      ...
                      >
                        <div className="news-story-topline">
                          <div className="news-story-tickers">
                            ${(item.sourceTickers || []).map(
                              (ticker) =>
                                html`<span key=${ticker} className="news-ticker-pill"
                                  >${ticker}</span
                                >`,
                            )}
                          </div>
                        ...
                        <div className="news-story-body">
                          ...
                          <div className=${`news-story-summary ${isExpanded ? "is-expanded" : ""}`}>
                        ...
                        <div className="news-story-footer">${renderFacetTags(item)}</div>
                      </article>
                    `;
                  })}
                </div>
