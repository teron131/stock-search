...
import { loadFreshNewsFeed, loadSharedNewsFeed, planNewsFeedLoad } from "./feed.js";
...
async function requestPortfolioNewsSummary({
  ...
  const timeoutId = setTimeout(() => controller.abort(), CONFIG.requestTimeoutMs.news);
  let responsePayload;
  try {
    const response = await fetch(CONFIG.endpoints.portfolioNewsSummarize, {
      ...
      body: JSON.stringify(buildPortfolioNewsSummaryRequestPayload(rows, allItems)),
  ...
  writePortfolioNewsSummaryCache(heldTickerKey, portfolioNewsSummary, new Date().toISOString());
  return portfolioNewsSummary;
}

/** Return portfolio news state, filters, refresh controls, and summary data for the UI. */
export function useNewsData({ rows, enabled, portfolioLoading = false, preferDemoData = false }) {
  const [feedState, setFeedState] = useState(INITIAL_FEED_STATE);
  const [tickerFilter, setTickerFilter] = useState("ALL");
  const [relevanceFilter, setRelevanceFilter] = useState("all");
  const [portfolioNewsSummaryResult, setPortfolioNewsSummaryResult] = useState(null);
  const {
    allItems,
    generatedAt,
    failedTickers,
  ...
  const applyNewsResult = useCallback(
    (newsResult, { demo = false, shared = false, partialError = false } = {}) => {
    ...
    async ({ background = false, force = false } = {}) => {
      ...
      try {
        if (!preferDemoData && !force) {
          const sharedNews = await loadSharedNewsFeed(heldTickers).catch(() => null);
          if (sharedNews) {
  ...
  const items = useMemo(
    () => sortNewsItems(filterNewsItems(allItems, { tickerFilter, relevanceFilter })),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const normalizedPortfolioNewsSummary = await requestPortfolioNewsSummary({
          allItems,
          fallbackPortfolioNewsSummary,
          heldTickerKey,
          rows,
  ...
  const portfolioNewsSummary = portfolioNewsSummaryResult || fallbackPortfolioNewsSummary;
