...
function normalizeDashboardRowsPayload(payload) {
  ...
  return {
    rows,
    generated_at: typeof payload.meta?.generated_at === "string" ? payload.meta.generated_at : null,
    portfolio_stats:
...
export function normalizeSectorPayload(payload) {
  ...
  return {
    ...
    meta: {
      source: typeof meta.source === "string" && meta.source ? meta.source : "stockanalysis",
      fetched_at: typeof meta.fetched_at === "string" && meta.fetched_at ? meta.fetched_at : null,
...
function normalizeNewsMetadata(metadata) {
  ...
  return {
    provider: typeof metadata.provider === "string" && metadata.provider ? metadata.provider : null,
    source_domain:
    ...
    published_at:
      typeof metadata.published_at === "string" && metadata.published_at
        ...
        : null,
    fetched_at:
      typeof metadata.fetched_at === "string" && metadata.fetched_at ? metadata.fetched_at : null,
  };
}
...
function normalizeNewsArticle(article, sourceTicker) {
  ...
  const url = typeof article.url === "string" && article.url.trim() ? article.url.trim() : null;
  if (!url) {
    return null;
  }
  ...
  return {
    url,
    title: typeof article.title === "string" && article.title.trim() ? article.title.trim() : url,
    date: typeof article.date === "string" && article.date.trim() ? article.date.trim() : null,
    days_ago: Number.isFinite(Number(article.days_ago)) ? Number(article.days_ago) : null,
    summary: typeof article.summary === "string" ? article.summary.trim() : "",
    relevancy: NEWS_RELEVANCY_LEVELS.has(article.relevancy) ? article.relevancy : "low",
    category: NEWS_CATEGORY_LEVELS.has(article.category) ? article.category : "other",
    sentiment: NEWS_SENTIMENT_LEVELS.has(article.sentiment) ? article.sentiment : "neutral",
...
export function normalizeTickerNewsPayload(payload, sourceTicker) {
  ...
  return payload.map((article) => normalizeNewsArticle(article, sourceTicker)).filter(Boolean);
}
  ...
  return {
    meta:
      payload.meta && typeof payload.meta === "object"
        ? {
            generated_at:
              typeof payload.meta.generated_at === "string" && payload.meta.generated_at
    ...
    items_by_ticker: Object.fromEntries(
      Object.entries(rawItemsByTicker).map(([ticker, articles]) => {
        ...
        return [normalizedTicker, normalizeTickerNewsPayload(articles, normalizedTicker)];
      }),
    ),
  };
}
...
function normalizeTickerNewsSummary(item) {
  ...
  const headline =
    typeof item.headline === "string" && item.headline.trim()
      ? item.headline.trim()
      : `${ticker} news`;
  const sourceUrls = normalizeTextList(readArrayProperty(item, "source_urls", "sourceUrls"));
  ...
  return {
    ...
    sourceUrls,
    status: typeof item.status === "string" && item.status ? item.status : "fresh",
  };
}
...
function normalizeEnumList({ values, fallbackValue, allowedValues, defaultValue }) {
...
export function normalizePortfolioNewsSummaryPayload(payload) {
  ...
  return {
    hasNews: typeof payload.has_news === "boolean" ? payload.has_news : Boolean(payload.hasNews),
    ...
    topTickers: rawTopTickers
      ...
      .map((item) => ({
        ...
        chapters: Array.isArray(item.chapters)
          ? item.chapters.map(normalizePortfolioNewsSummaryChapter).filter(Boolean)
          : [],
      }))
...
export function normalizePortfolioNewsPayload(payload) {
  ...
  const rawGroups = readArrayProperty(payload, "articles_by_ticker", "articlesByTicker");
  const generatedAt =
    typeof payload.refreshed_at === "string" && payload.refreshed_at
      ...
      : typeof payload.refreshedAt === "string" && payload.refreshedAt
        ? payload.refreshedAt
        : null;
  const tickerSummaries = readArrayProperty(payload, "ticker_summaries", "tickerSummaries")
  ...
  const groups = rawGroups
    ...
    .map((group) => {
      ...
      return {
        ticker,
        status: typeof group.status === "string" && group.status ? group.status : "fresh",
        articles: normalizeTickerNewsPayload(group.articles, ticker),
      };
    })
    .filter((group) => group.ticker);
  const summaryItems = tickerSummaries.map((item) => articleFromTickerSummary(item, generatedAt));
  const normalizedSummary = normalizePortfolioNewsSummaryPayload(payload.summary);
  ...
  return {
    ...
    failedTickers: Array.from(
      new Set([
        ...groups.filter((group) => group.status === "failed").map((group) => group.ticker),
        ...tickerSummaries.filter((item) => item.status === "failed").map((item) => item.ticker),
      ]),
    ),
    portfolioNewsSummary: normalizedSummary || summaryFromTickerSummaries(tickerSummaries),
  };
}
