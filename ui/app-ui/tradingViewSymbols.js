...
function isFundLikeInstrument(input, options) {
  const quoteType = normalizeTickerLabel(options.quoteType ?? input?.quote_type);
  const equityType = normalizeTickerLabel(options.equityType ?? input?.equity_type);
  return quoteType === "ETF" || equityType === "ETF" || equityType === "FUND";
}

function splitTradingViewPrefix(ticker) {
  const [prefix, symbol] = ticker.includes(":") ? ticker.split(":", 2) : ["", ticker];
...
export function getTradingViewTickerTagSymbol(input, options = {}) {
  ...
  if (!COMMON_US_SYMBOL_PATTERN.test(normalizedTicker)) {
    return US_SHARE_CLASS_SYMBOL_PATTERN.test(normalizedTicker) ? `NYSE:${normalizedTicker}` : "";
  }
...
export function buildTradingViewTickerTapeSymbols(rows, { limit, maxLength } = {}) {
  const symbols = [];
  const seen = new Set();
...
export function isTradingViewSymbolError(error) {
  const message =
    typeof error === "string" ? error : error?.message || error?.reason || String(error || "");
  return message.includes("[tv] no_such_symbol") || message.includes("[tv] invalid symbol");
}
