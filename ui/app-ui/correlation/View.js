...
function renderInsightCards(cards) {
  return html`
    <div className="correlation-insight-grid">
      ${cards.map(
        (card) => html`
          <div key=${card.key} className=${`correlation-insight-card is-${card.tone}`}>
            <div className="correlation-insight-icon">${renderIcon(card.icon)}</div>
...
function renderTradingViewTickerTag(ticker, variant) {
  const classPrefix = variant === "matrix" ? "correlation-matrix" : "correlation-rail";
  const symbol = getTradingViewTickerTagSymbol(ticker, { allowFunds: true });
  if (!symbol) {
    return html`
      <span className=${`${classPrefix}-ticker-fallback`} title=${ticker}> ${ticker} </span>
    `;
  }
  ...
  return tickers.map(
    (ticker, tickerIndex) => html`
      <foreignObject
        ...
        x=${isColumn ? axisSize + tickerIndex * cellSize : Math.max(0, axisSize - MATRIX_ICON_SIZE)}
        y=${isColumn ? Math.max(0, axisSize - MATRIX_ICON_SIZE) : axisSize + tickerIndex * cellSize}
  ...
  return tickers.flatMap((ticker, rowIndex) =>
    tickers.map((columnTicker, columnIndex) => {
      ...
      return html`
        ...
        >
          <rect
            className=${`correlation-matrix-cell ${rowIndex === columnIndex ? "is-diagonal" : ""}`}
            width=${cellSize}
            height=${cellSize}
          ...
          ${showCellNumbers
            ? html`
              ...
              `
            : null}
          <g className="correlation-cell-tooltip" transform=${`translate(${cellSize / 2}, -8)`}>
...
export function CorrelationView({ data, mode, setMode, isLoading, lastError, tickers }) {
  ...
  const tailRounded = payload?.tailMatrixPsd
    ...
    : EMPTY_MATRIX;
  const insightCards = buildInsightCards(payload?.normalMatrixRounded, tailRounded);
            ...
            ${MODES.map(
              (item) => html`
                <button
                  key=${item.value}
                  type="button"
                  className=${`correlation-mode-btn ${mode === item.value ? "active" : ""}`}
