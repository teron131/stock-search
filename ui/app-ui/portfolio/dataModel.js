...
function hasTopHoldingsProxyStats(row) {
  return (
    ...
    (Array.isArray(row?.proxied_stat_fields) && row.proxied_stat_fields.length > 0)
  );
}
...
export function mergeRows(dashData, evalData) {
  ...
  const evalMap = new Map(
    ensureEvalEntries(evalData).map((entry) => [normalizeTicker(entry.ticker), entry]),
  );
...
export function calculatePortfolioSummary(rows, portfolioStats) {
  ...
  const positionsFromApi = Number(portfolioStats.held_positions_count ?? portfolioStats.positions);
  ...
  return {
    ...
    change: {
      absolute: Number.isFinite(changeValueFromApi) ? changeValueFromApi : derived.change.absolute,
      percent: Number.isFinite(changePctFromApi) ? changePctFromApi : derived.change.percent,
    },
    positions: Number.isFinite(positionsFromApi) ? positionsFromApi : derived.positions,
    weightedBeta: Number.isFinite(weightedBetaFromApi) ? weightedBetaFromApi : null,
...
export function upsertRow(rows, nextRow) {
  ...
  const index = rows.findIndex((row) => normalizeTicker(row?.ticker) === ticker);
...
export function removeRow(rows, ticker) {
  ...
  return rows.filter((row) => normalizeTicker(row?.ticker) !== normalizedTicker);
}

export function getNormalizedPortfolioStats(dashData) {
  return dashData?.portfolio_stats && typeof dashData.portfolio_stats === "object"
