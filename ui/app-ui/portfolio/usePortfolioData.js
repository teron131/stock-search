...
const { initial: INITIAL_PORTFOLIO_SCOPE, live: LIVE_PORTFOLIO_SCOPE } = CONFIG.portfolioScopes;
...
function setValueIfChanged(setter, nextValue, areEqual = Object.is) {
  setter((currentValue) => (areEqual(currentValue, nextValue) ? currentValue : nextValue));
}
...
export function usePortfolioData() {
  ...
  const load = useCallback(
    async ({ background = false, silent = false, scope = LIVE_PORTFOLIO_SCOPE } = {}) => {
  ...
  const refreshPositionStats = useCallback(
    async (ticker) => {
      const rowPayload = await tryFetchJson(CONFIG.endpoints.stockStats(ticker));
      if (rowPayload?.row) {
        setRows((prevRows) => calculateRanks(replaceExistingRow(prevRows, rowPayload.row)));
        setPortfolioStats(null);
  ...
  const patchPortfolioPosition = useCallback(
    async ({ ticker, quantity, strategy }) => {
      ...
      const res = await fetch(`${CONFIG.endpoints.portfolio}/${normalizedTicker}`, {
        method: "PATCH",
      ...
      cancelSync();
      setRows((prevRows) => calculateRanks(mergePositionRow(prevRows, position)));
      setPortfolioStats(null);
