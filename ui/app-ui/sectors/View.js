...
import { getTradingViewTickerTagSymbol, normalizeTickerLabel } from "../tradingViewSymbols.js";
...
function renderIcon(iconName, className) {
  ...
  return html`
    <svg
      ...
      className=${className}
    >
      ${iconDefinition.paths.map((pathValue) => html`<path key=${pathValue} d=${pathValue} />`)}
    </svg>
  `;
}
...
function renderSectorHeader(column, sortKey, sortDirection, setSortKey) {
  ...
  if (!column.sortKey) {
    return html`<th key=${key} className=${column.headerClassName}>${column.label}</th>`;
  }
  return html`
    <th key=${key}>
      ${renderSortableHeader(column.label, column.sortKey, sortKey, sortDirection, setSortKey)}
    </th>
  `;
}
...
export function SectorView({
  ...
  const sectorColorMeta = calculateScoreColorMetadata(filteredSectors, SECTOR_COLOR_COLUMNS, {
      ...
      >
        <div className="sector-ledger-header">
          ...
          <div className="sector-ledger-status">
            ${isLoading ? "Refreshing snapshot..." : `${filteredSectors.length} sectors in scope`}
          </div>
        </div>
                  ...
                  <thead>
                    <tr>
                      ${SECTOR_TABLE_COLUMNS.map((column) =>
                        renderSectorHeader(column, sortKey, sortDirection, setSortKey),
                      )}
                    </tr>
                  </thead>
                  <tbody>
                    ${filteredSectors.map((sector) => {
                      const displaySectorName = getSectorDisplayName(sector.sector);
                      const marketCapValue = parseMarketCap(sector.market_cap);
                      return html`
                        <tr key=${sector.sector}>
                          <td className="sector-name-cell">
                            <span className="sector-name-ident">
                              ...
                              >
                                ${renderIcon(getSectorIconName(sector.sector), "sector-row-glyph")}
                              </span>
                          ...
                          <td>
                            ${renderConditionallyColoredValue(fmt.market_cap(sector.market_cap), {
                          ...
                          <td>
                            ${renderConditionallyColoredValue(fmt.number(sector.pe), {
                              value: sector.pe,
                              colorMeta: sectorColorMeta,
                              colorKey: "pe",
                            })}
                          </td>
                          <td className=${getToneClass(sector.profit_margin, "sector-tone")}>
                            ${renderConditionallyColoredValue(
                          ...
                          <td className=${getToneClass(sector.change_percent_1d, "sector-tone")}>
                            ${fmt.percent(sector.change_percent_1d)}
                          </td>
                          <td className=${getToneClass(sector.change_percent_1y, "sector-tone")}>
