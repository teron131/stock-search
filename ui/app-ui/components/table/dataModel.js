...
export function rowMatchesNormalizedSearch(row, normalizedQuery) {
  ...
  return [ticker, displayTicker, label, name].some((value) => value.includes(normalizedQuery));
}
...
function notionalTotal(value) {
  ...
  const total =
    Number(value.from_stocks ?? 0) + Number(value.from_etf ?? 0) + Number(value.from_options ?? 0);
...
export function isProxiedStatCell(row, key) {
  return Array.isArray(row?.proxied_stat_fields) && row.proxied_stat_fields.includes(key);
}
...
export function formatCellValue(row, col) {
  if (col.key === "market_cap" && isEtfLikeRow(row) && !isProxiedStatCell(row, col.key)) {
    return "--";
  }
