...
import { COLS, CONFIG, NOTIONAL_COLUMN_KEYS, WIDTH_GROUP_OPTIONS } from "../../config.js";
...
function syncHorizontalScroll(sourceEl, targetEl) {
  ...
  const nextScrollLeft = Math.min(Math.max(sourceEl.scrollLeft, 0), getMaxScrollLeft(targetEl));
  if (Math.abs(targetEl.scrollLeft - nextScrollLeft) <= SCROLL_SYNC_THRESHOLD_PX) {
    return false;
  }
...
function getVisibleRowWindow(wrapperEl, rowCount) {
  ...
  const start = Math.min(
    rowCount,
    Math.max(0, Math.floor((viewportTop - wrapperTop) / TABLE_ROW_HEIGHT_PX) - ROW_WINDOW_OVERSCAN),
  );
  const end = Math.min(
    rowCount,
    Math.max(
      0,
      Math.ceil((viewportBottom - wrapperTop) / TABLE_ROW_HEIGHT_PX) + ROW_WINDOW_OVERSCAN,
    ),
  );

  if (end <= start) {
    const fallbackStart = Math.max(0, Math.min(start, rowCount - ROW_WINDOW_INITIAL_COUNT));
    const fallbackEnd = Math.min(rowCount, fallbackStart + ROW_WINDOW_INITIAL_COUNT);
...
function weightedReturnPercent(rows, fieldName) {
  ...
  return priorValue > 0 ? ((currentValue - priorValue) / priorValue) * 100 : null;
}
...
function buildTopSectorTooltipRows(sectorDistribution) {
  return (Array.isArray(sectorDistribution) ? sectorDistribution : [])
    ...
    .filter(
      (sectorRow) => sectorRow.label && sectorRow.value != null && Math.round(sectorRow.value) > 0,
    )
...
function buildPortfolioSummaryRow(rows, stats, cols, tab) {
  ...
  for (const col of cols) {
    if (Object.hasOwn(summaryRow, col.key) || SUMMARY_BLANK_COLUMNS.has(col.key)) {
      continue;
    }
...
function renderCell({
  ...
  if (row.is_portfolio_summary) {
    if (key === "ticker") {
      const tooltipRows = Array.isArray(row.top_sector_rows) ? row.top_sector_rows : [];
  ...
  if (format === "percent") {
    ...
    content = html`<span className=${`badge ${badgeClass}`}>${formatCellValue(row, col)}</span>`;
  } else if (format === "percent_neutral" && key === "weight_pct" && row.etf_lookthrough_only) {
    content = html`<span className="cell-weight">--</span>`;
  } else if (format === "percent_neutral") {
    content = html`<span className="cell-weight">${formatCellValue(row, col)}</span>`;
    ...
    const scoreClass =
      numeric >= CONFIG.scoreThresholds.high
        ...
        : numeric <= CONFIG.scoreThresholds.low
          ? "score-low"
          : "score-mid";
    content = html`<span className=${scoreClass}>${formatCellValue(row, col)}</span>`;
  } else {
    content = formatCellValue(row, col);
  }

  if (isProxiedStatCell(row, key)) {
    content = html`<span className="cell-proxied-stat" title="Proxied from ETF top holdings"
      >${content}</span
    >`;
  }

  if (isDerivedStatCell(row, key)) {
    content = html`<span className="cell-derived-stat" title="Derived from PE and NTM forward PE"
      >${content}</span
    >`;
  }
...
export function Table({
  ...
  const showNotional =
    tableDisplayOptions?.showNotional ?? CONFIG.tableDisplayDefaults.showNotional;
  const cols = useMemo(
    () =>
      showNotional ? rawCols : rawCols.filter((col) => !NOTIONAL_COLUMN_KEYS.includes(col.key)),
  ...
  const filtered = useMemo(
    () =>
      rows.filter(
        (row) => rowBelongsToTab(row, tab) && (showNotional || !isGeneratedNotionalOnlyRow(row)),
      ),
  ...
  const targetRowIndex = useMemo(() => {
    ...
    return sorted.findIndex((row) => rowMatchesNormalizedSearch(row, normalizedSearchQuery));
  ...
  const columnCharCounts = useMemo(() => getColumnCharCounts(sorted, cols), [sorted, cols]);
  ...
  const tableWrapperClassName = [
    ...
    .filter(Boolean)
    .join(" ");
  const tableClassName = ["data-table", isEvaluationTab ? "data-table-evaluations" : ""]
    .filter(Boolean)
    .join(" ");
  const headerTableWidth = headerColumnWidths.reduce((totalWidth, width) => totalWidth + width, 0);
  ...
  const visibleRows = sorted.slice(effectiveRowWindow.start, effectiveRowWindow.end);
  const topSpacerHeight = effectiveRowWindow.start * TABLE_ROW_HEIGHT_PX;
  const bottomSpacerHeight = (sorted.length - effectiveRowWindow.end) * TABLE_ROW_HEIGHT_PX;
  const getBaseColumnStyle = (col) =>
    col.key === "ticker"
      ? null
      : getColumnWidthStyle(columnCharCounts[col.key], WIDTH_GROUP_OPTIONS[col.widthGroup] ?? {});
    ...
    html`<colgroup key="colgroup">
      ${Children.toArray(
        cols.map((col, colIndex) => {
          const className = [getColumnClassName(col.key), getColumnClusterClassName(col.cluster)]
            .filter(Boolean)
            .join(" ");
          return html`<col
            key=${col.key}
            className=${className}
            style=${useMeasuredWidths
  ...
  const renderHeaderTooltip = (c) => html`
    <span key="title" className="table-header-tooltip-title"> ${c.tooltip || c.label} </span>
    ${c.description
    ...
    ${c.tooltipRows?.length
      ? html`<span key="rows" className="table-header-tooltip-rows">
          ${Children.toArray(
            c.tooltipRows.map(
              (row, rowIndex) => html`
                <span key=${`${row.label}:${rowIndex}`} className="table-header-tooltip-row">
                  <span>${row.label}</span>
                  <span>${row.value}</span>
                </span>
              `,
  ...
  const renderHeaderCells = () =>
    Children.toArray(
      cols.map((c) => {
        const columnClassName = [getColumnClassName(c.key), getColumnClusterClassName(c.cluster)]
          .filter(Boolean)
          .join(" ");
        if (c.key === "remove") return html`<th key=${c.key} className=${columnClassName}></th>`;
        ...
        >
          ...
          >
            ...
            <span key="sort-indicator" className="sort-indicator" aria-hidden="true"></span>
            <span key="tooltip" className="table-header-tooltip" aria-hidden="true">
  ...
  const measureHeaderColumnWidths = useCallback(() => {
    ...
    if (!tableEl || !hasRows) {
      setHeaderColumnWidths((currentWidths) => (currentWidths.length === 0 ? currentWidths : []));
      return;
    }
    ...
    setHeaderColumnWidths((currentWidths) => {
      const isSameWidthSet =
        ...
        currentWidths.every((width, index) => Math.abs(width - nextWidths[index]) < 0.5);
    ...
    const updateRowWindow = () => {
      ...
      animationFrameId = window.requestAnimationFrame(() => {
        ...
        setRowWindow((currentWindow) =>
          rowWindowEqual(currentWindow, nextWindow) ? currentWindow : nextWindow,
        );
      });
    };
    ...
    Children.toArray(
      cols.map(
        (col, colIndex) => html`<td key=${col.key}>
          <span className=${`table-skeleton-cell ${colIndex === 0 ? "is-ticker" : ""}`}></span>
        </td>`,
      ),
    );
  ...
  const portfolioSummaryTable = portfolioSummaryRow
    ? html`<div
      ...
      >
        <table className=${`${tableClassName} data-table-summary`} style=${headerTableStyle}>
          ${renderColGroup({ useMeasuredWidths: true })}
          <tbody>
            <tr className="portfolio-summary-row" aria-label="Portfolio weighted summary">
  ...
  const bodyRows = Children.toArray(
    shouldShowLoadingRows
      ? skeletonRows.map(
          (_, rowIndex) => html`<tr key=${`loading-${rowIndex}`} className="table-skeleton-row">
            ${renderSkeletonCells()}
          </tr>`,
        )
      : hasRows
  ...
  return html`
    <div className="table-shell">
      ...
      >
        <table className=${`${tableClassName} data-table-header-clone`} style=${headerTableStyle}>
          ${renderColGroup({ useMeasuredWidths: true })}
