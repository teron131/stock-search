...
export function usePortfolioData() {
    ...
    const stats = useMemo(() => {
        ...
        const positions = weighted.filter(
        ...
        return {
            totalVal: Number.isFinite(totalFromApi)
                ? totalFromApi
                : derived.totalVal,
            change: {
            ...
            weightedBeta: Number.isFinite(weightedBetaFromApi)
                ? weightedBetaFromApi
                : null,
            weightedIv: Number.isFinite(weightedIvFromApi)
                ? weightedIvFromApi
                : null,
            sectorDistribution: Array.isArray(
    ...
    const startRealtimeSync = useCallback(async () => {
        ...
        const realtimeConfig = await tryFetchJson(
            CONFIG.endpoints.realtimeConfig,
        );
    ...
    const topTickers = useMemo(() => {
        return (
            [...rows]
                .sort(
                    (a, b) =>
                        (Number(b.weight_pct) || 0) -
                        (Number(a.weight_pct) || 0),
                )
                ...
                .filter(
                    (t, i, self) =>
                        t &&
                        t.length < CONFIG.maxTickerLength &&
                        self.indexOf(t) === i,
                )
